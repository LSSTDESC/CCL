...
def darkemu_Pgm_resp(cosmo, hmc, prof_hod, cM, deltah=0.02,
                     log10Mh_min=12.0,log10Mh_max=15.9,                               lk_arr=None, a_arr=None,
                     extrap_order_lok=1, extrap_order_hik=1,
                     use_log=False):
    """ Implements the response of galaxy-matter power spectrum to the long wavelength modes, described in         
    arXiv:2310.13330.
    """
    if lk_arr is None:
        status = 0
    ...
    for ia, aa in enumerate(a_arr):
        ...
        # mass function 
        dndlog10m_emu = ius(logMfor_hmf ,hmf(cosmo, 10**logMfor_hmf ,aa))  # Mpc^-3  
        ...
        for m in range(nM):                                                    
            ...
            b1_th_tink[m] = integrate.romb(dndlog10m_emu(logM1) * hbf(cosmo, (10 ** logM1), aa), \
                                dx = dlogM1) / integrate.romb(dndlog10m_emu(logM1), dx = dlogM1)

                    
        darkemu_set_cosmology(emu, cosmo_hp)
        for m in range(nM):
            Pnth_hp[m] = emu.get_phm(k_emu*(h/hp), np.log10(nths[m]*(1/hp)**3), z) * (1/hp)**3
            
        darkemu_set_cosmology(emu, cosmo_hm)
        for m in range(nM):
            Pnth_hm[m] = emu.get_phm(k_emu*(h/hm), np.log10(nths[m]*(1/hm)**3), z) * (1/hm)**3
        ...
        prof = prof_hod.fourier(cosmo, k_use, M, aa) 
        ...
        bgE = integrate.romb(dndlog10m_emu(logM) * Ng * \
            (hbf(cosmo, M, aa)), dx = dlogM, axis = 0) / ng

        bgE2 = integrate.romb(dndlog10m_emu(logM) * Ng * \
        b2H17(hbf(cosmo, M, aa)), dx = dlogM, axis = 0) / ng
        bgL = bgE - 1

        dndlog10m_func_mat = np.tile(dndlog10m_emu(logM), (len(k_emu), 1)).transpose()  # M_sol,Mpc^-3
        b1E_mat = np.tile((hbf(cosmo, M, aa)), (len(k_emu), 1)).transpose()
        ...
        Pgm = integrate.romb(dprof_dlogM * nP, \
              dx = dlogM, axis = 0) / ng   

        dnP_gm_db = integrate.romb(dprof_dlogM * (dnP_hm_db_emu), dx = dlogM, axis = 0) 

        #  Here we assume the galaxies' profile around host halo is fixed at physical corrdinate.
        dprof_dlogM_dlogk = np.zeros((nM,nk))
        for i in range(nM):
            dprof_dlogM_dlogk[i] = np.gradient((dprof_dlogM[i])) /  np.gradient(np.log(k_use))
        
        G_prof = +1./3. * integrate.romb(dprof_dlogM_dlogk * nP, dx = dlogM, axis = 0) 
        ...
        dPgm_db_lin = (47/21 + bgE2/bgE - bgE -1/3 * dpklin) * \
                        bgE * pk2dlin.__call__(k_use, aa, cosmo)
...
def darkemu_Pgg_resp(cosmo, hmc, prof_hod, deltalnAs=0.03,
                     log10Mh_min=12.0,log10Mh_max=15.9,          
                     lk_arr=None, a_arr=None,
                     extrap_order_lok=1, extrap_order_hik=1,
                     use_log=False):
    """ Implements the response of galaxy-auto power spectrum to the long wavelength modes, described in         
    arXiv:2310.13330.
    """   
    if lk_arr is None:
        status = 0
    ...
    for ia, aa in enumerate(a_arr):
        ...
        for m in range(nM):
            ...
            for n in range(nM):
                ...
                Pth_bin[m,n] = get_phh_massthreshold_mass(emu, k_emu, nths[m]/(h**3), Mh[n], z) * (1/h)**3
             
            
        darkemu_set_cosmology_forAsresp(emu, cosmo, deltalnAs)
        for m in range(nM):
            for n in range(nM):
                Pth_Ap[m,n] = emu.get_phh(k_emu, np.log10(nths[m]/(h**3)), np.log10(nths[n]/(h**3)), z) * (1/h)**3 

        darkemu_set_cosmology_forAsresp(emu, cosmo, -deltalnAs)
        for m in range(nM):
            for n in range(nM):
                Pth_Am[m,n] = emu.get_phh(k_emu, np.log10(nths[m]/(h**3)), np.log10(nths[n]/(h**3)), z) * (1/h)**3                   
        ...
        for i in range(nM):
            dprof_dlogM_dlogk[i] = np.gradient((dprof_dlogM[i])) /  np.gradient(np.log(k_use))
            dprof_1h_dlogk[i] = np.gradient((prof_1h[i])) /  np.gradient(np.log(k_use))
        ...
        #  Here we assume the galaxies' profile around host halo is fixed at physical corrdinate.
        G_prof = +2./3. * integrate.romb(
            resp_2h_int * nth_mat * dprof_dlogM_dlogk, axis=0, dx=dlogM)/ (ng ** 2) 
        ...
        #  Here we assume the galaxies' profile around host halo is fixed at physical corrdinate.
        G_prof = +1./3. * integrate.romb(dndlog10m_func_mat * dprof_1h_dlogk, dx = dlogM, axis = 0) / (ng ** 2)
...
def get_phh_massthreshold_mass(emu, k_emu, dens1, Mbin, redshift):     
    ...
    logdens1, logdens2p, logdens2m = np.log10(dens1), np.log10(dens2p), np.log10(dens2m)
...
def set_hmodified_cosmology(cosmo,deltah):
    ...
    cosmo_hp = cosmology.Cosmology(Omega_c=Omega_c_p,Omega_b=Omega_b_p,
                            h=hp, n_s=n_s, A_s=A_s)
                            
    cosmo_hm = cosmology.Cosmology(Omega_c=Omega_c_m,Omega_b=Omega_b_m,
