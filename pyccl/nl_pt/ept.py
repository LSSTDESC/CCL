...
class EulerianPTCalculator(CCLAutoRepr):
    ...
    def _check_init(self):
        if self.initialised:
            return
        raise CCLError(
            "PT templates have not been initialised "
    ...
    def update_ingredients(self, cosmo):
        ...
        kw = {
            "P": pklz0,
            "P_window": self.fastpt_par["P_window"],
    ...
    def _get_pgg(self, tr1, tr2):
        ...
        pgg = (
            ...
            + 0.25 * (b21 * b22)[:, None] * (Pd2d2 - 2.0 * s4)
            + 0.5 * (b11 * bs2 + b12 * bs1)[:, None] * Pd1s2
            + 0.25
            * (b21 * bs2 + b22 * bs1)[:, None]
            * (Pd2s2 - (4.0 / 3.0) * s4)
            + 0.25 * (bs1 * bs2)[:, None] * (Ps2s2 - (8.0 / 9.0) * s4)
        ...
        *,
        ...
        if t1 == "NC":
            ...
            if t2 == "NC":
                ...
                pk = self._get_pii(tracer1, tracer2, return_bb=return_ia_bb)
            else:  # Must be matter
    ...
    def get_pk2d_template(
        ...
        *,
        ...
        if return_ia_bb and (
            pk_name in ["c2:c2", "c2:cdelta", "cdelta:cdelta"]
        ):
            pk_name += "_bb"
