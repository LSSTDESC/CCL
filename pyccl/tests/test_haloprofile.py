...
def test_IA_halo_model():
    ...
    # lmax odd
    assert (ccl.halos.SatelliteShearHOD(concentration=cM, lmax=7)
            .lmax) % 2 == 0

    # Run with b!={0,2}
    assert (ccl.halos.SatelliteShearHOD(concentration=cM, b=-1.9, lmax=12)
...
def test_prefactor():
    ...
    hmc = ccl.halos.HMCalculator(mass_function=nM,
                                 halo_bias=bM, mass_def=hmd_200m)

    p = ccl.halos.HaloProfilePressureGNFW()  # a simple profile
    assert np.all(np.abs(p.get_normalization(cosmo, 1., hmc=hmc)-1.0 < 1e-10))

    p = ccl.halos.SatelliteShearHOD(concentration=cM)
