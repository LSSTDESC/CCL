...
def tkkaf(k1, k2, a, alpha=1.0, beta=1.0):
    return 1.0 / (k1**alpha * k2**beta)
...
def pred_covar(
    l1, l2, alpha=1.0, beta=1.0, prefac=1.0 / (4 * np.pi), chi_power=6
):
...
def test_cov_NG_sanity(alpha, beta, typ):
    ...
    if typ == "cNG":
        ...
        def cov_f(ll, **kwargs):
            return ccl.angular_cl_cov_cNG(
                COSMO, tr, tr, ell=ll, t_of_kk_a=tsp, **kwargs
            )

    elif typ == "SSC":
        a_s = np.linspace(0.1, 1.0, 1024)
        s2b = np.ones_like(a_s)
        cov_p = pred_covar(
            ls[None, :], ls[:, None], alpha, beta, prefac=1.0, chi_power=4
        )
    ...
    # Different ells
    cov = np.array([cov_f(ls, ell2=l) for l in ls])
    assert np.all(np.fabs(cov / cov_p - 1).flatten() < 1e-5)

    # Different ells (transpose)
    cov = np.array([cov_f(l, ell2=ls) for l in ls]).T
    assert np.all(np.fabs(cov / cov_p - 1).flatten() < 1e-5)

    # All scalars
    cov = np.array([[cov_f(l1, ell2=l2) for l1 in ls] for l2 in ls])
    assert np.all(np.fabs(cov / cov_p - 1).flatten() < 1e-5)
...
def test_cov_NG_errors(typ):
    ...
    with pytest.raises(ValueError):
        cov_f(
            COSMO, tr, tr, ell=ls, t_of_kk_a=tsp, integration_method="cag_cuad"
        )
...
def test_Sigma2B():
    ...
    s2b_c = np.array(
        [ccl.sigma2_B_disc(COSMO, a_arr=a, fsky=fsky) for a in a_use]
    )
    ...
    s2b_d = np.array(
        [simpson(integrand(lk_arr, a, R), x=lk_arr) for a, R in zip(a_use, Rs)]
    )
    ...
    a_s, s2b = ccl.sigma2_B_from_mask(COSMO, a_arr=None, mask_wl=mask_wl)
