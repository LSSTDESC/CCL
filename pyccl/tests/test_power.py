...
@pytest.mark.parametrize(
    "k", [1, 1.0, [0.3, 0.5, 10], np.array([0.3, 0.5, 10])]
)
...
def test_linear_power_raises():
    ...
    with pytest.raises(KeyError):
        ccl.linear_power(cosmo, 1.0, 1.0, p_of_k_a="a:b")


def test_nonlin_power_raises():
    cosmo = ccl.CosmologyVanillaLCDM(transfer_function="bbks")
    with pytest.raises(KeyError):
        ccl.nonlin_power(cosmo, 1.0, 1.0, p_of_k_a="a:b")
...
@pytest.mark.parametrize(
    "r", [1, 1.0, [0.3, 0.5, 10], np.array([0.3, 0.5, 10])]
)
...
@pytest.mark.parametrize("A", [1, 1.0, [0.3, 0.5, 1], np.array([0.3, 0.5, 1])])
def test_kNL(A):
    knl = ccl.kNL(COSMO, A)
...
@pytest.mark.parametrize(
    "tf", ["boltzmann_class", "boltzmann_camb", "boltzmann_isitgr"]
)
def test_power_sigma8norm_norms_consistent(tf):
    ...
    pk_rat = ccl.linear_matter_power(cosmo, 1e-4, a) / ccl.linear_matter_power(
        cosmo_s8, 1e-4, a
    )
...
def test_input_lin_power_spectrum():
    # Setup
    cosmo = ccl.Cosmology(
        Omega_c=0.27, Omega_b=0.05, h=0.7, n_s=0.965, A_s=2e-9
    )
    ...
    assert np.allclose(pk_CCL_input, pk_CCL, atol=0.0, rtol=1e-5)
    ...
    pk_CCL_input = ccl.power.linear_power(
        cosmo_input, k_arr, 0.5, p_of_k_a="a:b"
    )
    assert np.allclose(pk_CCL_input, pk_CCL, atol=0.0, rtol=1e-5)
...
def test_input_nonlinear_model():
    ...
    pk_CCL_input = cosmo_input.get_nonlin_power("a:b")(k_arr, 0.5, cosmo_input)
    assert np.allclose(pk_CCL_input, pk_CCL, atol=0.0, rtol=1e-5)

    # Via `nonlin_power`
    pk_CCL_input = ccl.power.nonlin_power(
        cosmo_input, k_arr, 0.5, p_of_k_a="a:b"
    )
    assert np.allclose(pk_CCL_input, pk_CCL, atol=0.0, rtol=1e-5)
    ...
    cosmo_input = ccl.CosmologyCalculator(
        ...
        nonlinear_model={"a:b": "halofit", "c:d": None},
    )
    pk_CCL_input = ccl.power.nonlin_power(
        cosmo_input, k_arr, 0.5, p_of_k_a="a:b"
    )
...
def test_input_nonlinear_model_raises():
    ...
    with pytest.raises(ValueError):
        ccl.CosmologyCalculator(
            ...
            n_s=0.965,
            sigma8=0.8,
            pk_nonlin={"a": a_arr, "k": k_arr, "a:b": pkl_arr},
        )
