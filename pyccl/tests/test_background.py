...
AVALS = [1, 1.0, 0.8, [0.2, 0.4], np.array([0.2, 0.4])]
...
def test_angular_distance_arrs():
    ...
    d = ccl.angular_diameter_distance(
        COSMO, np.full(10, a_lens), np.full(10, a_source)
    )
    assert np.all(np.fabs(d - d0) < 1e-10)
...
def test_input_arrays_raises():
    ...
    with pytest.raises(TypeError):
        ccl.CosmologyCalculator(
            Omega_c=0.27, Omega_b=0.05, h=0.7, n_s=0.965, A_s=2e-9, growth=3
        )
    with pytest.raises(TypeError):
        ccl.CosmologyCalculator(
            Omega_c=0.27,
    ...
    with pytest.raises(KeyError):
        ccl.CosmologyCalculator(
            ...
            n_s=0.965,
            A_s=2e-9,
            background={"a": input_a_array, "h_over_h0": input_hoh0},
        )
    with pytest.raises(KeyError):
        ccl.CosmologyCalculator(
            Omega_c=0.27,
            ...
            n_s=0.965,
            A_s=2e-9,
            growth={"a": input_a_array, "growth_rate": input_fgrowth},
        )
