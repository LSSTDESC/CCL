...
def test_resample_raises():
    ...
    with pytest.raises(ValueError):
        ccl.resample_array(R_ARR, R_ARR, R_ARR, "linx_lagy", "linx_logy")
    with pytest.raises(ValueError):
        ccl.resample_array(R_ARR, R_ARR, R_ARR, "linx_logy", "linx_lagy")
...
def test_resample_extrapolation(extrap):
    ...
    if extrap == "none":
        with pytest.raises(ccl.CCLError):
            ccl.resample_array(
                R_ARR, f_arr, r_arr_x, extrap, extrap, offset, offset
            )
    else:
        f_arr_x = ccl.resample_array(
            R_ARR, f_arr, r_arr_x, extrap, extrap, offset, offset
        )
