...
def test_pk2d_init():
    ...
    lkarr = -4.0 + 6 * np.arange(100) / 99.0
    aarr = 0.05 + 0.95 * np.arange(100) / 99.0
...
@pytest.mark.parametrize(
    "model", ["bbks", "eisenstein_hu", "eisenstein_hu_nowiggles"]
)
...
def test_pk2d_function():
    ...
    psp = ccl.Pk2D.from_function(
        pkfunc=lpk2d, spline_params=cosmo.cosmo.spline_params
    )
    ...
    psp = ccl.Pk2D.from_function(
        pkfunc=pk2d, is_logp=False, spline_params=cosmo.cosmo.spline_params
    )
    phere = psp(ktest, atest, cosmo)
    assert np.allclose(phere, ptrue, atol=0, rtol=1e-6)
    dphere = psp(ktest, atest, cosmo, derivative=True)
    ...
    assert np.allclose(phere, ptrue, atol=0, rtol=1e-6)
    dphere = psp(ktest, atest, cosmo, derivative=True)
    assert np.allclose(dphere, -1.0 * np.ones_like(dphere), atol=0, rtol=1e-6)
...
def test_pk2d_from_function_spline_params():
    ...
    pk2 = ccl.Pk2D.from_function(
        pk2d, is_logp=False, spline_params=cosmo.cosmo.spline_params
    )
    ...
    pk3 = ccl.Pk2D.from_function(
        pk2d, is_logp=False, spline_params=ccl.spline_params
    )
...
def test_pk2d_parsing():
    ...
    pk_arr = (
        a_arr[:, None] * ((k_arr / 0.01) / (1 + (k_arr / 0.01) ** 3))[None, :]
    )
    ...
    cells1 = ccl.angular_cl(
        cosmo, lens1, lens1, ells, p_of_k_a="delta_matter:delta_matter"
    )
    cells2 = ccl.angular_cl(cosmo, lens1, lens1, ells, p_of_k_a="a:b")
    cells3 = ccl.angular_cl(cosmo, lens1, lens1, ells, p_of_k_a=psp)
...
def test_pk2d_add():
    ...
    empty_pk2d = ccl.Pk2D.__new__(ccl.Pk2D)
    pk2d_a = ccl.Pk2D(
        a_arr=x, lk_arr=log_y, pk_arr=np.log(zarr_a), is_logp=True
    )
    pk2d_b = ccl.Pk2D(a_arr=2 * x, lk_arr=log_y, pk_arr=zarr_b, is_logp=False)
    pk2d_b2 = ccl.Pk2D(
        a_arr=x, lk_arr=log_y + 0.5, pk_arr=zarr_b, is_logp=False
    )
    ...
    pk2d_c = ccl.Pk2D(a_arr=x, lk_arr=log_y, pk_arr=zarr_b, is_logp=False)
...
def test_pk2d_mul_pow():
    ...
    pk2d_b = ccl.Pk2D(a_arr=x, lk_arr=log_y, pk_arr=zarr_b, is_logp=False)
...
def test_pk2d_eval_cosmo():
    ...
    assert pk(1.0, 1.0) == pk(1.0, 1.0, cosmo)

    amin = pk.psp.amin
    pk(1.0, amin * 0.99, cosmo)  # doesn't fail because cosmo is provided
    with pytest.raises(ValueError):
        pk(1.0, amin * 0.99)
...
def test_pk2d_copy():
    ...
    pkc = pk.copy()
    assert np.allclose(
        pk.get_spline_arrays()[-1], pkc.get_spline_arrays()[-1], rtol=1e-15
    )
...
def test_pk2d_operations():
    ...
    # rsub, rtruediv
    assert np.allclose(
        (1 - pk1).get_spline_arrays()[-1], 1 - pk1.get_spline_arrays()[-1]
    )
    assert np.allclose(
        (1 / pk1).get_spline_arrays()[-1], 1 / pk1.get_spline_arrays()[-1]
    )
