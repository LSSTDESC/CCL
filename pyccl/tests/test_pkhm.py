...
P1 = ccl.halos.HaloProfileNFW(
    mass_def=M200, concentration=CON, fourier_analytic=True
)
P2 = P1
...
def smoke_assert_pkhm_real(func):
    ...
    for (sa, sk), sh in zip(sizes, shapes):
        if sk == 0:
            ...
            k = np.logspace(-2.0, 0.0, sk)
        if sa == 0:
            a = 1.0
        else:
            a = np.linspace(0.5, 1.0, sa)
        p = func(k, a)
...
def test_HMIngredients_eq_repr_hash():
    ...
    HMC3 = ccl.halos.HMCalculator(
        mass_function="Press74", halo_bias="Sheth01", mass_def="fof"
    )  # not eq
...
def test_pkhm_pk2d():
    ...
    pk2d = ccl.halos.halomod_Pk2D(
        COSMO, hmc, P1, lk_arr=np.log(k_arr), a_arr=a_arr
    )
    ...
    G1 = ccl.halos.HaloProfileHOD(
        mass_def=M200, concentration=CON, log10Mmin_0=12.00000
    )
    G2 = ccl.halos.HaloProfileHOD(
        mass_def=M200, concentration=CON, log10Mmin_0=11.99999
    )
    assert G1 != G2

    # I_1_1
    pk0 = ccl.halos.halomod_power_spectrum(
        COSMO, hmc, k_arr, a_arr, G1, prof2=G1
    )
    pk1 = ccl.halos.halomod_power_spectrum(
        COSMO, hmc, k_arr, a_arr, G1, prof2=G2
    )
    ...
    # I_0_2 & I_1_2
    assert np.allclose(
        hmc.I_0_2(COSMO, KK, AA, P1, prof_2pt=PKC),
        hmc.I_0_2(COSMO, KK, AA, P1, prof2=P1, prof_2pt=PKC),
        rtol=0,
    )
    assert np.allclose(
        hmc.I_1_2(COSMO, KK, AA, P1, prof_2pt=PKC),
    ...
    pk0 = ccl.halos.halomod_power_spectrum(
        COSMO, hmc, k_arr, a_arr, P1, smooth_transition=None
    )
    pk1 = ccl.halos.halomod_power_spectrum(
        COSMO, hmc, k_arr, a_arr, P1, smooth_transition=alpha0
    )
    assert np.allclose(pk0, pk1, rtol=0)
    pk2 = ccl.halos.halomod_power_spectrum(
        COSMO, hmc, k_arr, a_arr, P1, smooth_transition=alpha1
    )
    assert np.all(pk2 / pk0 > 1.0)
    ...
    pk0 = ccl.halos.halomod_power_spectrum(
        COSMO, hmc, k_arr, a_arr, P1, suppress_1h=None, get_2h=False
    )
    pk1 = ccl.halos.halomod_power_spectrum(
        COSMO, hmc, k_arr, a_arr, P1, suppress_1h=ks0, get_2h=False
    )
    assert np.allclose(pk0, pk1, rtol=0)
    pk2 = ccl.halos.halomod_power_spectrum(
        COSMO, hmc, k_arr, a_arr, P1, suppress_1h=ks1, get_2h=False
    )
    assert np.allclose(pk2, 0, rtol=0)
    pk3 = ccl.halos.halomod_power_spectrum(
        COSMO, hmc, k_arr, a_arr, P1, suppress_1h=ks2, get_2h=False
    )
...
def test_pkhm_errors():
    ...
    with pytest.raises(ValueError):
        ccl.halos.HMCalculator(
            mass_function=hmf, halo_bias=hbf, mass_def=m200c
        )
    ...
    with pytest.raises(ValueError):
        ccl.halos.halomod_power_spectrum(
            COSMO, hmc, KK, AA, P1, smooth_transition=func, get_1h=False
        )

    # Wrong 1h damping
    with pytest.raises(ValueError):
        ccl.halos.halomod_power_spectrum(
            COSMO, hmc, KK, AA, P1, suppress_1h=func, get_1h=False
        )
