...
@pytest.mark.parametrize(
    "isNC1,isNC2,isNC3,isNC4", itertools.product([True, False], repeat=4)
)
...
def get_ssc_counterterm_gc(k, a, hmc, prof1, prof2, prof12_2pt):
    ...
    if prof1.is_number_counts or prof2.is_number_counts:
        ...
        i02_12 = hmc.I_0_2(
            COSMO, k, a, prof1, prof_2pt=prof12_2pt, prof2=prof2
        ) / (norm1 * norm2)
...
@pytest.mark.parametrize(
    ...
    [
        ...
        {"prof": NFW, "prof2": NFW, "prof3": NFW, "prof4": NFW},
        {"prof": HOD, "prof2": NFW, "prof3": NFW, "prof4": NFW},
        {"prof": HOD, "prof2": HOD, "prof3": NFW, "prof4": NFW},
        {"prof": HOD, "prof2": HOD, "prof3": HOD, "prof4": NFW},
        {"prof": NFW, "prof2": NFW, "prof3": HOD, "prof4": HOD},
        {"prof": HOD, "prof2": NFW, "prof3": HOD, "prof4": HOD},
        {"prof": HOD, "prof2": HOD, "prof3": NFW, "prof4": HOD},
        {"prof": HOD, "prof2": None, "prof3": NFW, "prof4": None},
        {"prof": HOD, "prof2": None, "prof3": None, "prof4": None},
        {"prof": HOD, "prof2": HOD, "prof3": None, "prof4": None},
        # As in benchmarks/test_covariances.py
        {"prof": NFW, "prof2": NFW, "prof3": None, "prof4": None},
        # Setting prof34_2pt
        {
            "prof": NFW,
            "prof2": NFW,
        ...
        {"prof": HOD, "prof2": HOD, "prof3": HOD, "prof4": HOD},
    ],
)
def test_tkkssc_counterterms_gc(kwargs):
    ...
    tkk_nogc = ccl.halos.halomod_Tk3D_SSC(
        COSMO, HMC, lk_arr=np.log(k_arr), a_arr=a_arr, **kwargs_nogc
    )
    _, _, _, tkk_nogc_arrs = tkk_nogc.get_spline_arrays()
    tk_nogc_12, tk_nogc_34 = tkk_nogc_arrs

    # Tk's with clustering terms
    tkk_gc = ccl.halos.halomod_Tk3D_SSC(
        COSMO, HMC, lk_arr=np.log(k_arr), a_arr=a_arr, **kwargs
    )
    ...
    for aa in a_arr:
        tkc12.append(
            get_ssc_counterterm_gc(
                k_arr, aa, HMC, kwargs["prof"], kwargs["prof2"], PKC
            )
        )
        tkc34.append(
            get_ssc_counterterm_gc(
                k_arr, aa, HMC, kwargs["prof3"], kwargs["prof4"], PKC
            )
        )
