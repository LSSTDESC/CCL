...
def test_get_Delta():
    for Delta in [200, 500, 121]:
        ...
        assert int(hmd.get_Delta(COSMO, 1.0)) == Delta

    hmd = ccl.halos.MassDef("vir", "critical")
    assert np.isfinite(hmd.get_Delta(COSMO, 1.0))

    hmd = ccl.halos.MassDef("fof", "critical")
    with pytest.raises(ValueError):
        hmd.get_Delta(COSMO, 1.0)


def test_get_mass():
    hmd = ccl.halos.MassDef(200, "critical")
    for R in [1.0, [1.0, 2.0], np.array([1.0, 2.0])]:
        m = hmd.get_mass(COSMO, R, 1.0)
...
def test_get_radius():
    hmd = ccl.halos.MassDef(200, "critical")
    for M in [1e12, [1e12, 2e12], np.array([1e12, 2e12])]:
        r = hmd.get_radius(COSMO, M, 1.0)
...
def test_translate_mass():
    ...
    translator = ccl.halos.mass_translator(
        mass_in=hmd, mass_out=hmdb, concentration=cm
    )
    for M in [1e12, [1e12, 2e12], np.array([1e12, 2e12])]:
...
def test_mass_translator():
    ...
    with pytest.raises(ValueError):
        ccl.halos.mass_translator(
            mass_in=mdef1, mass_out=mdef2, concentration=cm
        )
