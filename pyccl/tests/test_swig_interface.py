...
def test_swig_tracer():
    ...
    with pytest.raises(ValueError):
        pyccl.WeakLensingTracer(
            PYCOSMO, dndz=(z, dNdz[0:2]), ia_bias=(z, bias_ia)
        )

    with pytest.raises(ValueError):
        pyccl.WeakLensingTracer(
            PYCOSMO, dndz=(z, dNdz), ia_bias=(z, bias_ia[0:2])
        )
...
def test_swig_correlation():
    ...
    with pytest.raises(CCLError):
        ccllib.correlation_vec(
            COSMO, [1, 2], [1, 2, 3], [0, 1, 2, 3], 0, 0, 4, status
        )

    with pytest.raises(CCLError):
        ccllib.correlation_vec(
            COSMO, [1, 2, 3], [1, 2, 3], [0, 1, 2, 3], 0, 0, 5, status
        )
...
def test_swig_neurtinos():
    ...
    with pytest.raises(CCLError):
        ccllib.Omeganuh2_vec(
            3, 2.7, 0.72, [0.0, 1.0], [0.05, 0.1, 0.2], 4, status
        )

    with pytest.raises(CCLError):
        ccllib.Omeganuh2_vec(3, 2.7, 0.72, [0.0, 1.0], [0.1, 0.2], 2, status)
