...
TRS = {
    "TG": ccl.nl_pt.PTNumberCountsTracer(b1=2.0, b2=2.0, bs=2.0),
    "TI": ccl.nl_pt.PTIntrinsicAlignmentTracer(c1=2.0, c2=2.0, cdelta=2.0),
    "TM": ccl.nl_pt.PTMatterTracer(),
}
PTC = ccl.nl_pt.EulerianPTCalculator(
    with_NC=True, with_IA=True, with_matter_1loop=True, cosmo=COSMO
)


def test_ept_calculator_smoke():
    c = ccl.nl_pt.EulerianPTCalculator(
        log10k_min=-3, log10k_max=1, nk_per_decade=10, pad_factor=2
    )
    assert len(c.k_s) == 40
...
def test_ept_k2pk_types(typ_nlin, typ_nloc):
    tg = ccl.nl_pt.PTNumberCountsTracer(1.0, 0.0, 0.0, bk2=1.0)
    tm = ccl.nl_pt.PTNumberCountsTracer(1.0, 0.0, 0.0)
    ptc1 = ccl.nl_pt.EulerianPTCalculator(
        with_NC=True,
        with_IA=True,
        with_matter_1loop=True,
        b1_pk_kind=typ_nlin,
        bk2_pk_kind=typ_nloc,
        cosmo=COSMO,
    )
    ...
    p1 = pkgg(ks, 1.0, cosmo=COSMO)
    p2 = pkmm(ks, 1.0, cosmo=COSMO) + ks**2 * pkmm2(ks, 1.0, cosmo=COSMO)
    ...
    def get_tr(tn):
        ...
        if tn in b_ia:
            ...
            return ccl.nl_pt.PTIntrinsicAlignmentTracer(
                c1=bdict["c1"], c2=bdict["c2"], cdelta=bdict["cdelta"]
            )
...
@pytest.mark.parametrize("kind", ["c2:c2", "c2:cdelta", "cdelta:cdelta"])
...
def test_ept_matter_1loop():
    ...
    # as matter P(k)
    ptc1 = ccl.nl_pt.EulerianPTCalculator(
        with_NC=False, with_IA=False, with_matter_1loop=True, cosmo=COSMO
    )
...
def test_ept_matter_linear():
    ...
    ptc1 = ccl.nl_pt.EulerianPTCalculator(
        with_matter_1loop=True, b1_pk_kind="linear", cosmo=COSMO
    )
    pk1 = ptc1.get_biased_pk2d(TRS["TM"])(ks, 1.0, cosmo=COSMO)
    ptc2 = ccl.nl_pt.EulerianPTCalculator(
        with_matter_1loop=True, b1_pk_kind="pt", cosmo=COSMO
    )
...
def test_ept_calculator_raises():
    ...
    with pytest.raises(ValueError):
        ptc = ccl.nl_pt.EulerianPTCalculator(with_NC=False, cosmo=COSMO)
        ptc.get_biased_pk2d(TRS["TG"])

    with pytest.raises(ValueError):
        ptc = ccl.nl_pt.EulerianPTCalculator(with_IA=False, cosmo=COSMO)
    ...
    with pytest.warns(ccl.CCLWarning):
        ptc = ccl.nl_pt.EulerianPTCalculator(
            with_NC=True, with_IA=True, cosmo=COSMO
        )
...
def test_ept_eq():
    ...
    ptc2 = ccl.nl_pt.EulerianPTCalculator(
        with_NC=True, with_IA=True, with_matter_1loop=True, C_window=0.5
    )
    assert ptc1 != ptc2
