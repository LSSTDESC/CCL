...
def test_pt_tracer_nc_smoke(b2):
    pt_tr = ccl.nl_pt.PTNumberCountsTracer((ZZ, BZ), b2=b2, bs=(ZZ, BZ))
...
def test_pt_tracer_ia_smoke(c2):
    pt_tr = ccl.nl_pt.PTIntrinsicAlignmentTracer(
        (ZZ, BZ), c2=c2, cdelta=(ZZ, BZ)
    )
...
def test_pt_tracer_get_bias():
    pt_tr = ccl.nl_pt.PTNumberCountsTracer((ZZ, BZ), b2=(ZZ, BZ), bs=(ZZ, BZ))
