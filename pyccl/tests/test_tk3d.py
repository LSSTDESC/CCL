...
def get_arrays(islog=True):
    ...
    tkka_arr = np.array(
        [tkkaf(k_arr[None, :], k_arr[:, None], a) for a in a_arr]
    )
    if islog:
        fka1_arr = np.log(fka1_arr)
        fka2_arr = np.log(fka2_arr)
        tkka_arr = np.log(tkka_arr)
    return (a_arr, lk_arr, fka1_arr, fka2_arr, tkka_arr)
...
def test_tk3d_errors():
    ...
    with pytest.raises(ValueError):
        ccl.Tk3D(
            a_arr=a_arr, lk_arr=lk_arr, tkk_arr=tkka_arr, extrap_order_hik=-1
        )
    with pytest.raises(ValueError):
        ccl.Tk3D(
            a_arr=a_arr, lk_arr=lk_arr, tkk_arr=tkka_arr, extrap_order_lok=2
        )
...
def test_tk3d_smoke():
    ...
    (a_arr, lk_arr, fka1_arr, fka2_arr, tkka_arr) = get_arrays()
    tsp1 = ccl.Tk3D(
        a_arr=a_arr, lk_arr=lk_arr, pk1_arr=fka1_arr, pk2_arr=fka2_arr
    )
...
def test_tk3d_call():
    ...
    assert np.allclose(
        np.array([tsp(np.exp(lk_arr), a) for a in a_arr]),
        tsp(np.exp(lk_arr), a_arr),
        rtol=1e-15,
    )
