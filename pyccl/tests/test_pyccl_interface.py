...
def reference_models():
    ...
    return [cosmo1, cosmo4, cosmo5, cosmo9]
...
def check_halomod(cosmo):
    ...
    assert_raises(
        TypeError, ccl.halo_concentration, cosmo, mass_scalar, a_array, odelta)
    assert_raises(
        TypeError, ccl.halo_concentration, cosmo, mass_list,   a_array, odelta)
    assert_raises(
        TypeError, ccl.halo_concentration, cosmo, mass_array,  a_array, odelta)
    ...
    assert_raises(
        TypeError, ccl.halomodel_matter_power, cosmo, k_scalar, a_array)
    assert_raises(
        TypeError, ccl.halomodel_matter_power, cosmo, k_list,   a_array)
    assert_raises(
        TypeError, ccl.halomodel_matter_power, cosmo, k_array,  a_array)
...
def check_haloprofile(cosmo):
    ...
    # halo model
    assert_(all_finite(
        ccl.nfw_profile_3d(cosmo, c, mass_scalar, odelta, a, r_scalar)))
    assert_(all_finite(
        ccl.nfw_profile_3d(cosmo, c, mass_scalar, odelta, a, r_list)))
    assert_(all_finite(
        ccl.nfw_profile_3d(cosmo, c, mass_scalar, odelta, a, r_array)))

    assert_(all_finite(
        ccl.einasto_profile_3d(cosmo, c, mass_scalar, odelta, a, r_scalar)))
    assert_(all_finite(
        ccl.einasto_profile_3d(cosmo, c, mass_scalar, odelta, a, r_list)))
    assert_(all_finite(
        ccl.einasto_profile_3d(cosmo, c, mass_scalar, odelta, a, r_array)))

    assert_(all_finite(
        ccl.hernquist_profile_3d(cosmo, c, mass_scalar, odelta, a, r_scalar)))
    assert_(all_finite(
        ccl.hernquist_profile_3d(cosmo, c, mass_scalar, odelta, a, r_list)))
    assert_(all_finite(
        ccl.hernquist_profile_3d(cosmo, c, mass_scalar, odelta, a, r_array)))

    assert_(all_finite(
        ccl.nfw_profile_2d(cosmo, c, mass_scalar, odelta, a, r_scalar)))
    assert_(all_finite(
        ccl.nfw_profile_2d(cosmo, c, mass_scalar, odelta, a, r_list)))
    assert_(all_finite(
        ccl.nfw_profile_2d(cosmo, c, mass_scalar, odelta, a, r_array)))

    assert_raises(
        TypeError, ccl.nfw_profile_3d,
        cosmo, c, mass_array, odelta, a, r_scalar)
    assert_raises(
        TypeError, ccl.nfw_profile_3d, cosmo, c, mass_array, odelta, a, r_list)
    assert_raises(
        TypeError, ccl.nfw_profile_3d,
        cosmo, c, mass_array, odelta, a, r_array)

    assert_raises(
        TypeError, ccl.nfw_profile_3d,
        cosmo, c, mass_scalar, odelta, a_array, r_scalar)
    assert_raises(
        TypeError, ccl.nfw_profile_3d,
        cosmo, c, mass_scalar, odelta, a_array, r_list)
    assert_raises(
        TypeError, ccl.nfw_profile_3d,
        cosmo, c, mass_scalar, odelta, a_array, r_array)
...
def check_cls(cosmo):
    ...
    # assert_(all_finite(
    #     ccl.angular_cl(cosmo, nc1, nc1, ell_arr, l_limber=20)))
    # Non-Limber only implemented for number counts
    # Uncomment once we bring ANGPOW back
    # assert_raises(CCLError,
    #               ccl.angular_cl, cosmo, lens1, lens1, ell_arr, l_limber=20)
...
def check_cls_nu(cosmo):
    ...
    # Check that for massive neutrinos including rsd raises
    # an error (not yet implemented)
    assert_raises(
        CCLError, ccl.NumberCountsTracer, cosmo,
...
def test_background():
    for cosmo in reference_models():
        check_background(cosmo)


def test_background_nu():
    for cosmo_nu in reference_models_nu():
        check_background_nu(cosmo_nu)


def test_background_mg():
    for cosmo_mg in reference_models_mg():
        check_background(cosmo_mg)
...
def test_power_nu():
    for cosmo_nu in reference_models_nu():
        check_power(cosmo_nu)


def test_power_mg():
    for cosmo_mg in reference_models_mg():
        check_power_MG(cosmo_mg)


def test_massfunc():
    for cosmo in reference_models():
        check_massfunc(cosmo)


def test_massfunc_nu():
    for cosmo_nu in reference_models_nu():
        check_massfunc_nu(cosmo_nu)
...
def test_halomod_mg():
    for cosmo_mg in reference_models_mg():
        check_massfunc_MG(cosmo_mg)


def test_haloprofile():
    for cosmo in reference_models():
        check_haloprofile(cosmo)
...
def test_corr3d_nu():
    for cosmo_nu in reference_models_nu():
        check_corr_3d(cosmo_nu)


def test_corr3d_mg():
    for cosmo_mg in reference_models_mg():
        check_corr_3d(cosmo_mg)


def test_corr3drsd():
    for cosmo in reference_models():
        check_corr_3dRSD(cosmo)


def test_corr3drsd_nu():
    for cosmo_nu in reference_models_nu():
        check_corr_3dRSD(cosmo_nu)
