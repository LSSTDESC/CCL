...
def halomod_trispectrum_1h(cosmo, hmc, k, a, prof, *,
    ...
    if np.ndim(k) == 0:
        ...
        out = np.squeeze(out, axis=-1)
    # FAO: resolve normalization factor of ~98.35 * reference 
    return out


def halomod_trispectrum_2h_22(cosmo, hmc, k, a, prof, *,
                              prof2=None,
    ...
    # FAO: check new profiles definition 
    prof, prof2, prof3, prof4, prof12_2pt, prof34_2pt = \
        _allocate_profiles(prof, prof2, prof3, prof4, prof12_2pt, prof34_2pt)
    ...
    def get_isotropized_pkr(p_of_k_a, aa):
        ...
        # FAO: changing input:
        pk = np.loadtxt(f'{ref_path}/Plin_exp_kr.txt').reshape((nk, nk, theta.size))
...
#         # FAO changing input
#         # ref_path = '.../onecov' 
#         p1 = np.loadtxt(f'{ref_path}/Plin_exp.txt')[:, None]
#         # OAF
...
#         # LJF
#         pk = np.loadtxt(f'{ref_path}/Plin_exp.txt')[:, None]
#         pkr = np.loadtxt(f'{ref_path}/Plin_exp_kr.txt').reshape((nk, nk, theta.size))
#         # LJF termina
#         # LJF dividi por pi a linha de baixo !!!!
...
#         # LJF salvando PT bispectro
#         # out_path = '/home/lucas/programs/RobertReiscke_covcode/onecov'
#         np.savetxt(f'{out_path}/Bpt_ccl.txt', Bpt)
#         # LJF termina
...
#         # FAO: LJF
#         print(get_pk(kr.flatten(), aa).shape)
#         print(get_pk(kr.flatten(), aa).reshape((nk, nk, theta.size)).shape) 
#         pk = np.loadtxt(f'{ref_path}/Plin_exp.txt')[:, None]
#         pkr = np.loadtxt(f'{ref_path}/Plin_exp_kr.txt').reshape((nk, nk, theta.size))
#         # LJF termina
...
#         # FAO: LJF Saving PT trispectrum
#         np.savetxt(f'{out_path}/Tpt_ccl.txt', (t1113 + t1122))
#         np.savetxt(f'{out_path}/Plin_ccl.txt', get_pk(k_use, aa))
#         # LJF termina
