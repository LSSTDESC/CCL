...
def _compute_p_hh(ks, Mh, redshift):
...
# referred from dark_emulator_public/dark_emulator/model_hod/hod_interface.py
def _compute_p_hm(ks, Mh, redshift):
    if not hod.logdens_computed:
        hod._compute_logdens(redshift)
    if not hod.xi_hm_computed:
...
def darkemu_power_spectrum(demu, cosmo, hmc, k, a, prof, *,
    ...
    if np.any(good_a is False):
        warnings.warn('Dark Emulator I supports 0 <= z <= 1.48. '
                      'The redshifts rather than 1.48 will be truncated.')
    a_use = a_use[good_a]
        ...
        #     u2k = prof2.fourier(cosmo, k_use, M_array, aa).T
            ...
            for i in range(nM):
                for j in range(i+1):
                    ...
                    pkhh[:, i, j] = vals
                    pkhh[:, j, i] = vals
            # pkhh = _compute_p_hh(k_use, M_array, redshift=1./aa-1)
            # (Nk,NM,NM)
