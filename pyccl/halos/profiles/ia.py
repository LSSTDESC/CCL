...
class SatelliteShearHOD(HaloProfileHOD):
    ...
    def __init__(
        ...
        *,
        ...
        self.a1h = a1h
        self.b = b
        if integration_method not in ["FFTLog", "simpson", "spline"]:
            raise ValueError(
                "Integration method provided not "
        ...
        if self.b == 0:
            ...
            self._angular_fl = np.array(
                [self._fl(l, b=self.b) for l in range(2, self.lmax + 1, 2)]
    ...
    def _usat_fourier(self, cosmo, k, M, a):
        ...
        if not self.integration_method == "FFTLog":
            ...
            x_jn = np.geomspace(
                k_use.min() * r_use.min(), k_use.max() * r_use.max(), self.N_jn
            )
        ...
        for j, l in enumerate(l_arr):
            if self.integration_method == "FFTLog":
                prof += (
                    self._fftlog_wrap(
                        cosmo, k_use, M_use, a, ell=int(l), fourier_out=True
                    )
                    * (1j**l).real
                    * (2 * l + 1)
                    * self._angular_fl[j]
                ...
                integrand = (
                    r_use**2 * jn_interp * self._real(cosmo, r_use, M_use, a)
                )
