...
void ccl_correlation_3dRsd_vec(ccl_cosmology *cosmo,double a,double mu,double beta,
			       int ns,double *s,
                               double *xis,int nxis,int do_avg_mu,
         		       int *status)
{ 
  assert(ns==nxis);

  ccl_correlation_3dRsd(cosmo,a,ns,s,mu,beta,xis,do_avg_mu,status);
}
%}
