...
def get_camb_pk_lin(cosmo, *, nonlin=False):
    ...
    if np.isfinite(cosmo["A_s"]):
        ...
        raise CCLError(
            ...
            "A_s = %f, sigma8 = %f" % (cosmo["A_s"], cosmo["sigma8"])
        )
    ...
    g = np.power(cosmo["T_ncdm"] / np.power(11.0 / 4.0, -1.0 / 3.0), 4.0)
    ...
    cp.YHe = cp.bbn_predictor.Y_He(
        cp.ombh2 * (camb.constants.COBE_CMBTemp / cp.TCMB) ** 3, delta_neff
    )
    ...
    if camb_de_model not in camb_de_models:
        raise ValueError(
            "The only dark energy models CCL supports with "
            "CAMB are fluid and ppf."
        )
    cp.set_classes(dark_energy_model=camb_de_model)

    if (
        camb_de_model not in camb_de_models[:2]
        and cosmo["wa"]
        and (cosmo["w0"] < -1 - 1e-6 or 1 + cosmo["w0"] + cosmo["wa"] < -1e-6)
    ):
        raise ValueError(
            "If you want to use w crossing -1,"
    ...
    if nonlin:
        ...
        options = {
            k: extra_camb_params[k]
            for k in [
                "HMCode_A_baryon",
                "HMCode_eta_baryon",
                "HMCode_logT_AGN",
            ]
            if k in extra_camb_params
        }
        cp.NonLinearModel.set_params(
            halofit_version=halofit_version, **options
        )
...
def get_isitgr_pk_lin(cosmo):
    ...
    cp.YHe = cp.bbn_predictor.Y_He(
        cp.ombh2 * (isitgr.constants.COBE_CMBTemp / cp.TCMB) ** 3, delta_neff
    )
    ...
    cp.DarkEnergy.set_params(w=cosmo["w0"], wa=cosmo["wa"])
    # cp.set_cosmology()
    cp.set_matter_power(redshifts=[_z for _z in zs], kmax=10, nonlinear=False)
...
def get_class_pk_lin(cosmo):
    ...
    try:
        ...
        lk_arr = np.log(
            np.logspace(
                -5, np.log10(cosmo.cosmo.spline_params.K_MAX_SPLINE), nk
            )
        )
