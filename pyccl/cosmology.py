...
= 1e-5``.
    ...
    def __init__(
            ...
            bcm_log10Mc=None, bcm_etab=None, bcm_ks=None, mu_0=0., sigma_0=0.,
    ...
    def __enter__(self):
        warnings.warn("Context manager functionality is deprecated.",
                      CCLDeprecationWarning)
        return self
    ...
    def get_linear_power(self, name=DEFAULT_POWER_SPECTRUM):
        ...
        """
        return self._pk_lin[name] if name in self._pk_lin else None
    ...
    def get_nonlin_power(self, name=DEFAULT_POWER_SPECTRUM):
        ...
        Returns:
            ...
            power spectrum with name `name`.
        """
        return self._pk_nl[name] if name in self._pk_nl else None
    ...
    def has_linear_power(self):
        ...
        try:
            return isinstance(self._pk_lin[DEFAULT_POWER_SPECTRUM], Pk2D)
        except KeyError:
            return False
    ...
    def has_nonlin_power(self):
        ...
        try:
            return isinstance(self._pk_nl[DEFAULT_POWER_SPECTRUM], Pk2D)
        except KeyError:
            return False
...
class CosmologyCalculator(Cosmology):
    ...
    def __init__(
        ...
        if pk_nonlin is not None:
            self._init_pk_nonlin(pk_nonlin, nonlinear_model)
    ...
    def _check_input(self, a, arr1, arr2):
        ...
        if not (np.diff(a) > 0).all():
            raise ValueError("Scale factor is not monotonically increasing.")
    ...
    def _init_pk_linear(self, pk_linear):
        ...
        for name in pk_names:
            ...
            use_log = (pk > 0).all()
            if use_log:
                np.log(pk, out=pk)  # log in-place

            self._pk_lin[name] = Pk2D(a_arr=pk_linear['a'], lk_arr=lk,
                                      pk_arr=pk, is_logp=use_log)

    def _init_pk_nonlin(self, pk_nonlin, nonlinear_model):
        ...
        for name in pk_names:
            ...
            self._pk_nl[name] = Pk2D(a_arr=pk_nonlin['a'], lk_arr=lk,
                                     pk_arr=pk, is_logp=use_log)
