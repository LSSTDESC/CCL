...
import warnings
import numpy as np
from . import DEFAULT_POWER_SPECTRUM, CCLWarning, CCLError, check, lib
...
def nonlimber_FKEM(cosmo,clt1, clt2, p_of_k_a, ls, l_limber, limber_max_error):
	...
	for t in clt1._trc: status = lib.add_cl_tracer_to_collection(t1, t, status)
	for t in clt2._trc: status = lib.add_cl_tracer_to_collection(t2, t, status)
	...
	for i in range(len(kernels_t1)):
		...
		fchi1_arr[i] = fchi1_interp(chi_logspace_arr) * chi_logspace_arr*growfac_arr * transfer_t1_low[i]/transfer_t1_avg[i]
	...
	for j in range(len(kernels_t2)):
		...
		fchi2_arr[j] = fchi2_interp(chi_logspace_arr) * chi_logspace_arr*growfac_arr* transfer_t2_low[j]/transfer_t2_avg[j]
	...
	for el in range(len(ls)):
		...
		status=0
		cl_limber_lin, status = lib.angular_cl_vec_limber(cosmo.cosmo, t1, t2, psp_lin, [ell], integ_types['qag_quad'],1, status)
		if status != 0: raise ValueError("Error in Limber integrator.")
		cl_limber_nonlin, status = lib.angular_cl_vec_limber(cosmo.cosmo, t1, t2, psp_nonlin, [ell], integ_types['qag_quad'],1, status)
		if status != 0: raise ValueError("Error in Limber integrator.")
		...
		for i in range(len(kernels_t1)):
			...
			k, fk1 = _fftlog_transform_general(chi_logspace_arr, fchi1_arr[i], float(ell), nu, 1, float(deriv), float(plaw))
			...
			for j in range(len(kernels_t2)):
				...
				if clt1!=clt2:
					...
					k, fk2 = _fftlog_transform_general(chi_logspace_arr, fchi2_arr[j], float(ell), nu2, 1, float(deriv2), float(plaw2))
		...
		cells.append(cl_limber_nonlin[-1]- cl_limber_lin[-1] + cls_nonlimber_lin)
		if (np.abs(cells[-1]/cl_limber_nonlin[-1] - 1)<limber_max_error and l_limber=='auto') or (type(l_limber) !=str and  ell >=l_limber):
			l_limber = ell
