...
class amemuLinear(EmulatorPk):
    def __init__(self, download: bool = True):
        ...
        Calculates the linear matter power spectrum based on the following setup:
        - redshift: [0.0, 5.0]
    ...
    def _get_pk_at_a(self, cosmo: dict, a: np.ndarray) -> Tuple[np.ndarray, np.ndarray]:
        """
        Calculates the linear matter power spectrum given a dictionary for cosmology and the scale factor. An example of
        ...
        Returns:
            Tuple[np.ndarray, np.ndarray]: the wavenumbers and the linear matter spectrum at the pre-defined values of k.
        """
        redshift = (1.0 - a) / a
        record_mean = []
        for z in redshift:
            pk_quant = self.emulator.calculate_pklin(z, cosmo, return_var=False)
    ...
    def _get_pk2d(self, cosmo: dict) -> Pk2D:
        """
        Calculates the 2D linear matter spectrum, as a function of wavenumber (1/Mpc) and scale factor.
        The scale factors are for redshift between 0 and 5, inclusive.
        ...
        redshifts = np.linspace(self.emulator.config.Z_MIN + 1e-10, self.emulator.config.Z_MAX - 1e-10, 21)
