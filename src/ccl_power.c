...
static void ccl_cosmology_compute_linpower_analytic(
  ...
  if(*status==0) {
    cosmo->data.p_lin=ccl_f2d_t_new(na,z,nk,x,y2d,NULL,NULL,0,0,0,
...
static void ccl_cosmology_compute_power_emu(ccl_cosmology * cosmo, int * status)
{
  ...
  *status=CCL_ERROR_MEMORY;
  ...
  if(*status==0) {
    cosmo->data.p_nl=ccl_f2d_t_new(na,aemu,NK_EMU,lk,lpk_nl,NULL,NULL,0,0,0,
...
static void ccl_cosmology_spline_nonlinpower(
  ...
  if(*status == 0)
    cosmo->data.p_nl = ccl_f2d_t_new(na, z, nk, x, y2d, NULL, NULL, 0,0,0,
...
void ccl_cosmology_compute_power(ccl_cosmology* cosmo, int* status)
{
  ...
  switch (cosmo->config.transfer_function_method) {
    ...
    case ccl_eisenstein_hu: {
        eh_struct *eh = ccl_eh_struct_new(&(cosmo->params),1);
