...
//clt1 -> tracer #1
//clt2 -> tracer #2
static double ccl_angular_cl_native(ccl_cosmology *cosmo, double l,
  ...
  if(*status==0) {
    ...
    for(ii=0;ii<w->n_ls;ii++) {
      if((!do_angpow) || (w->l_arr[ii] > w->l_limber))
        cl_nodes[ii] = ccl_angular_cl_native(cosmo, w->l_arr[ii], clt1, clt2, status);
    }
