...
static void ccl_tracer_corr_fftlog(ccl_cosmology *cosmo,
  ...
  ccl_fftlog_ComputeXi2D(i_bessel,0,
			 cosmo->spline_params.N_ELL_CORR,l_arr,cl_arr,
			 th_arr,wth_arr);
...
void ccl_correlation_3d(ccl_cosmology *cosmo, double a,
  ...
  ccl_fftlog_ComputeXi3D(0, 0, N_ARR, k_arr, pk_arr, r_arr, xi_arr);
...
void ccl_correlation_multipole(ccl_cosmology *cosmo, double a, double beta,
  ...
  if (l == 0) {
    ccl_fftlog_ComputeXi3D(0, 0, N_ARR, k_arr, pk_arr, s_arr, xi_arr0);
    for (i = 0; i < N_ARR; i++)
      xi_arr[i] = (1. + 2. / 3 * beta + 1. / 5 * beta * beta) * xi_arr0[i];
  } else if (l == 2) {
    ccl_fftlog_ComputeXi3D(2, 0, N_ARR, k_arr, pk_arr, s_arr, xi_arr0);
    for (i = 0; i < N_ARR; i++)
      xi_arr[i] = -(4. / 3 * beta + 4. / 7 * beta * beta) * xi_arr0[i];
  } else if (l == 4) {
    ccl_fftlog_ComputeXi3D(4, 0, N_ARR, k_arr, pk_arr, s_arr, xi_arr0);
...
void ccl_correlation_multipole_spline(ccl_cosmology *cosmo, double a,
  ...
  // Calculate multipoles
  ccl_fftlog_ComputeXi3D(0, 0, N_ARR, k_arr, pk_arr, s_arr, xi_arr0);
  ccl_fftlog_ComputeXi3D(2, 0, N_ARR, k_arr, pk_arr, s_arr, xi_arr2);
  ccl_fftlog_ComputeXi3D(4, 0, N_ARR, k_arr, pk_arr, s_arr, xi_arr4);
