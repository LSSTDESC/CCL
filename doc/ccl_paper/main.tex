...
\asv{I think this is sufficient given sigma(M) is merely sigma(R->M) with the
following translation above.}
...
\end{equation}
where $A$, $a$, $b$, and $c$ are fitting parameters that have additional redshift scaling and $\sigma$ is the RMS variance of the density field smoothed on some scale $M$ at some scale factor $a$ \todo{there are two different $a$'s in the above equation, can we rename one?}\asv{I am personally against the choice to change variable names here, as this matches the form presented in Tinker 2008.}. This basic form is modified for the \citet{Angulo2012} formulation. The resulting form is
\begin{equation}
...
The \citet{Tinker2010} model parameterizes the halo mass function in terms of the peak height, $\nu = \delta_c/\sigma(M)$, where $\delta_c=1.686$ is the critical density for collapse. The function is then written as\todo{Give it a different name?$\tilde{f}$?}\asv{Also against giving this function a different name, as it refers to the same fundamental thing, except as a function of nu instead of a function of sigma(M).}
\begin{equation}
  f(\nu) = \alpha[1+(\beta\nu)^{-2\phi}]\nu^{2\eta}e(-\gamma\nu^2/2).
\end{equation}

We note that these halo mass functions, while implemented to high {\em numerical} accuracy through comparison against a flat $\Lambda$CDM cosmology (see CCL1 in \ref{tab:cosmologies}, carry their own uncertainties. It has not been significantly studied whether the halo mass function (or halo bias function) is universal with respect to changes in dark energy parameterisation.
