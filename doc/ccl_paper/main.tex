...
\item Lensing $E$-mode angular power spectra: leading order term only (no magnification or intrinsic alignments), on the same scales.
\item The cross-power spectrum between both quantities (galaxy-galaxy lensing), on the same scales.
...
\end{itemize}
Notice that intrinsic alignments, RSD and magnification predictions are not currently validated. We do not include real-space (correlation function) benchmarks involving CMB lensing, since they are functionally the same as number counts (i.e. a spin-0 quantity).
...
\caption{Tests of the angular power spectrum accuracy. {\sl Top:} benchmark comparisons for power spectra between number counts in pairs of redshift bins. Results are shown for analytic (solid) and binned (dashed) redshift distributions, for the bin pairs 1-1 (blue), 1-2 (orange) and 2-2 (green). {\sl Middle:} same as the top panel for cross-correlations between number counts and weak lensing. {\sl Bottom:} same as before for the weak lensing power spectra.}
\label{fig:cls_limber}
\end{figure}
\begin{figure}
\includegraphics[width=0.49\textwidth]{Cl_dc}
\includegraphics[width=0.49\textwidth]{Cl_lc}
\caption{Same as Figure \ref{fig:cls_limber} for power spectra involving CMB lensing. {\sl Top:} cross-correlations with number counts and CMB auto-correlation. {\sl Bottom:} cross-correlations with weak lensing.}
