...
\end{equation}
where $a_{\ell m}$/$b_{\ell m}$ can be either the $E$-mode or $B$-mode component of the corresponding field. In what follows we will only deal with fields for which the $B$-modes are identically $0$, and therefore all equations refer to the $E$-$E$ power spectrum. In general this power spectrum can be written as:
\begin{equation}
...
\begin{itemize}
  ...
  \item $\Delta^{\rm IA}$ is the transfer function for intrinsic galaxy alignments. \ccl currently supports the so-called ``non-linear alignment model'', in which the intrinsic galaxy inertia tensor is proportional the local tidal tensor \cite{2004PhRvD..70f3526H,2007MNRAS.381.1197H}:
    ...
    \end{align}
    Here $b_{\rm IA}$ is the so-called alignment bias, and $f_{\rm red}$ is the fraction of red galaxies in the sample. It is understood that only red galaxies are subject to this type of alignment mechanism, therefore $f_{\rm red}$ should be interpreted as ``the fraction of galaxies that suffer intrinsic alignment''. 
\end{itemize}
...
\end{equation}
where $d^\ell_{mm'}$ are the Wigner-$d$ matrices\footnote{\url{https://en.wikipedia.org/wiki/Wigner_D-matrix}}
\citep{Ng1999,2004MNRAS.350..914C} and we have defined the power spectra
\begin{equation}
  C^{ab\pm}_\ell\equiv\left(C^{a_Eb_E}_\ell\pm C^{a_Bb_B}_\ell\right)+i\left(C^{a_Bb_E}_\ell\pm C^{a_Eb_B}_\ell\right),
\end{equation}
which reduces to the $E$-$E$ power spectrum when all $B$-modes are 0.

Note that, scalar quantities are real, any correlation involving at least one spin-$0$ field only has one non-unique correlation function. In these cases, the Wigner-$d$ matrices can also be expressed in terms of associated Legendre polynomials $P^m_\ell$, and therefore Eq. \ref{eq:cl_xi} becomes
\begin{align}
...
In addition to the angular correlation functions, \ccl can also be used to compute the 3-dimensional spatial correlation function $\xi(r)$, given as a Fourier transform of the matter power spectrum:
\begin{equation}
\xi(r) = \frac{1}{2 \pi^2} \int dk \; k^2 P(k) \frac{\sin(kr)}{kr}
\label{eq:xi3d}
\end{equation}
In the future \ccl will be expanded to incorporate the calculation of the higher-order multipoles needed to characterize the redshift-space 3D correlation function in the presence of RSDs.
