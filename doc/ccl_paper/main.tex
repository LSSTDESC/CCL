...
%\elisa{aren't these already given above?}
%\Mead{Moved this}
...
%\elisa{I think we can't say ``any'', we don't provide a halo model for galaxy shapes, for example. Let's specify the types of fields: matter and linearly biased tracers.}
%\Mead{Done}
...
%\elisa{do we know more or less the level of physical accuracy behind this assumption?}
%\Mead{Done}
...
%\elisa{define what's two- and one-halo}
%\Mead{Done}
...
%\elisa{what is the correct large-scale limit?}
%\Mead{Done}
...
\begin{equation}
\begin{split}
W_\delta(M,k) = &\, 4\pi r_\mathrm{s}^3 \\%\left\{ \\
...
- & \frac{\sin(kr_\mathrm{v})}{(1+c)kr_\mathrm{s}}\ ,%\right\}\ ,
