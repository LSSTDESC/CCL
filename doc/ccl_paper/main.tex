...
\end{itemize}
 Besides these two-point functions, we also validated the computation of the CMB lensing power spectrum against an independent implementation, using the same $\ell$ binning described above. \todo{Although we did not directly validate the computation of cross-correlations between number counts and convergence, the only difference of this case with respect to other real space correlation functions is the Bessel function present in the integrand. Hence, we deemed unnecessary to perform further validations.} Notice that intrinsic alignments, RSD and aagnification predictions are not currently validated. 
