...
\begin{equation}
  \mathcal{A} \equiv \frac{|D_{\tt CCL}(z)-D_{i}(z)|}{D_\ccl(z)}
\end{equation}
\todo{Check for all $\mathcal{A}$ definitions whether \ccl is in the denominator in the code.}
...
\begin{equation}
  \mathcal{A}\equiv \frac{|P_{\tt CCL}(k,z)-P_{i}(k,z)|}{P_{\tt CCL}(k,z)}
  \label{eq:pkacc}
\end{equation}
...
\begin{equation}
  \mathcal{A}\equiv\frac{|P_{\tt CCL}(k,z)-P_{\rm L17}(k,z)|}{P_{\tt CCL}(k,z)}
  \label{eq:pkaccemu}
\end{equation}
...
Independent codes were utilized to test the accuracy of halo mass function predictions. These predictions are computed from power spectra obtained using the BBKS approximation. For the halo mass function, we compare the value of $\sigma$, $\tilde\sigma\equiv\log(\sigma^{-1})$, and the value of the halo mass function in the form used in \cite{Tinker2008},
\begin{equation}
\mathcal{H}\equiv \log[(M^2/\bar{\rho}_m)dn/dM].
\end{equation}
These defines three new accuracy metrics:
\begin{eqnarray}
  \mathcal{A}_{hmf1}&\equiv&\frac{|\sigma_{\tt CCL}-\sigma_i|}{\sigma_{\tt CCL}},\\
  \mathcal{A}_{hmf2}&\equiv&\frac{|\tilde\sigma_{\tt CCL}-\tilde\sigma_i|}{\tilde\sigma_{\tt CCL}},\\
  \mathcal{A}_{hmf3}&\equiv&\frac{|\mathcal{H}_{\tt CCL}-\mathcal{H}_i|}{\mathcal{H}_{\tt CCL}}.
\end{eqnarray}
...
\begin{equation}
  \mathcal{A}=\left|\frac{C_\ell^{\tt CCL}-C_\ell^{(i)}}{C_\ell^{\tt CCL}}\right|.
\end{equation}
We achieved a relative difference between \ccl and the benchmarks of $<10^{-3}$. This applied both to analytic redshift distributions and histograms, for auto- and cross-correlations between bins for both galaxy clustering and weak lensing, as shown in Figure \ref{fig:cls_limber}. The CMB lensing result is shown in Figure \ref{fig:cls_cmblens}.\elisa{I think we have a fraction of allowed failures as well - need to check in code.}
