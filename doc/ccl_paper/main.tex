...
\begin{align}
  ...
  % \label{eq:Ylm_Pl}.
\end{align}
Using the addition theorem for spherical harmonics and the relation between spherical harmonics and
wigner D-matrices\footnote{https://en.wikipedia.org/wiki/Wigner_D-matrix} \citep{Ng1999}, we can
write
\begin{align}
  \langle XZ
...
The galaxy number count is spin zero and shear is a spin-2 object. Thus the expressions for various correlations and cross correlations are
\begin{align}
...
\begin{align}
  C_{\ell}^{g\kappa}&={\frac{(\ell+1)!}{(\ell-1)!}}C_{\ell}^{gm}\\
  C_{\ell}^{g\gamma}&=\sqrt{\frac{(\ell+2)!}{(\ell-2)!}}C_{\ell}^{gm}
  \\ \langle g\gamma_T\rangle(\theta)&
