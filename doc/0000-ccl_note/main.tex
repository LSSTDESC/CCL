...
The computation of the Fourier transform for $\hat{\gamma}_s^I(\mathbf{k}|M)$ is done using the FFTLog algorithm by default. Another method available is the Simpsons integration and we have checked that angular power spectra computed with both methods agree very well, even when the density of the sampling points for the Simpsons method is very high. This ensures that the FFTLog algorithm provides accurate angular power spectra with very high computational efficiency. 
