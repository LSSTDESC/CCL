...
int main(int argc,char **argv)
{
  ...
  printf("z\tOmega_m\tOmega_L\tOmega_r\trho_crit\trho_crit_comoving\trho_m\trho_m_comoving\tRHO_CRITICAL\n");
  double Om, OL, Or, rhoc, rhoc_comoving, rhom, rhom_comoving;
  for (int z=10000;z!=0;z/=3){
    ...
    rhoc = ccl_rho_x(cosmo, 1./(z+1), ccl_rho_crit_label, &status);
    rhoc_comoving = ccl_rho_x(cosmo, 1./(z+1), ccl_rho_crit_comoving_label, &status);
    rhom = ccl_rho_x(cosmo, 1./(z+1), ccl_rho_m_label, &status);
    rhom_comoving = ccl_rho_x(cosmo, 1./(z+1), ccl_rho_m_comoving_label, &status);
    printf("%i\t%.3f\t%.3f\t%.3f\t%.3e\t%.3e\t%.3e\t%.3e\t%.3e\n", z, Om, OL, Or, rhoc, rhoc_comoving, rhom, rhom_comoving, RHO_CRITICAL);
  }
  ...
  rhoc = ccl_rho_x(cosmo, 1., ccl_rho_crit_label, &status);
  rhoc_comoving = ccl_rho_x(cosmo, 1., ccl_rho_crit_comoving_label, &status);
  rhom = ccl_rho_x(cosmo, 1., ccl_rho_m_label, &status);
  rhom_comoving = ccl_rho_x(cosmo, 1., ccl_rho_m_comoving_label, &status);
  printf("%i\t%.3f\t%.3f\t%.3f\t%.3e\t%.3e\t%.3e\t%.3e\t%.3e\n", 0, Om, OL, Or, rhoc, rhoc_comoving, rhom, rhom_comoving, RHO_CRITICAL);
