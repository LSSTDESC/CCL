...
------------------------------------------------------------
 ...
 7. After making changes to any of these files, developers should run:
      $> autoreconf -i
 ...
 9. Any updates to CCL should be documented in the CCL note that you can find 
    in `doc/0000-ccl_note/`. You will need to edit `main.tex`, compile with: 
    $> make
...
Notes on autotools:
 - To install the library, users will run:
     ...
     $> make
     $> make install
 - Once the library is compiled, all unit tests are run by typing:
     $> make check
...
2.5. Still not working? You can execute `python setup.py install [args] --record files.txt`. 
