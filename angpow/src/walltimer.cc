...
#include <cmath>
#include <algorithm>
#include "Angpow/walltimer.h"
#include "Angpow/walltime_c.h"

#include "Angpow/angpow_exceptions.h" //for assert method
...
void tstack_pop(const string &name)
  {
  double t0=wallTime();
  angpow_assert(curnode && (curnode->name==name), "invalid tstack operation");
  double t1=wallTime();
...
void tstack_pop()
  {
  double t0=wallTime();
  angpow_assert(curnode, "invalid tstack operation");
  double t1=wallTime();
  curnode->wt.stop(0.5*(t0+t1));
  curnode=curnode->parent;
  overhead+=t1-t0;
...
void tstack_replace(const string &name1, const string &name2)
  {
  angpow_assert(curnode && (curnode->name==name1), "invalid tstack operation");
...
void tstack_report(const string &stem)
  ...
  angpow_assert(ptr,"invalid stem");
