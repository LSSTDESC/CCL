...
static void check_transfer_error(ccl_configuration config, struct power_MG_data * data)
{
  ...
  if (config.matter_power_spectrum_method == ccl_emu){
    ASSERT_STR(cosmo->status_message,
               "ccl_power.c: ccl_cosmology_compute_power(): The power spectrum "
               "in the mu / Sigma modified gravity parameterisation is only "
               "implemented with the linear power spectrum.\n");
  }
  else {
    ASSERT_STR(cosmo->status_message,
               "ccl_power.c: ccl_cosmology_compute_power(): The power spectrum "
               "in the mu / Sigma modified gravity parameterisation is only "
               "implemented with the ccl_boltzmann_class power spectrum method.\n");
  }
...
static void check_nonlin_error(struct power_MG_data * data)
{
  ...
  ASSERT_STR(cosmo->status_message,
             "ccl_power.c: ccl_cosmology_compute_power(): The power "
             "spectrum in the mu / Sigma modified gravity parameterisation "
...
CTEST2(power_MG, MG_emu_error) {
  ...
  ccl_configuration config_emu = {
    ccl_transfer_none, ccl_emu, ccl_nobaryons,
...
CTEST2(power_MG, MG_eh_error) {
  ...
  ccl_configuration config_eh = {
    ccl_eisenstein_hu, ccl_linear, ccl_nobaryons,
    ccl_tinker10, ccl_duffy2008, ccl_emu_strict};
...
CTEST2(power_MG, MG_bbks_error) {
  ...
  ccl_configuration config_bbks = {
    ccl_bbks, ccl_linear, ccl_nobaryons,
    ccl_tinker10, ccl_duffy2008, ccl_emu_strict};
