...
def test_hbf_tinker10():
    d_hbf = np.loadtxt(os.path.join(dirdat, "hbf_tinker10.txt"), unpack=True)
    mf = ccl.halos.HaloBiasTinker10()
    m = d_hbf[0]
    for iz, z in enumerate(zs):
        hb_d = d_hbf[iz + 1]
        hb_h = mf(cosmo, m, 1.0 / (1 + z))
        assert np.all(np.fabs(hb_h / hb_d - 1) < 1e-3)


def test_hbf_sheth01():
    d_hbf = np.loadtxt(os.path.join(dirdat, "hbf_sheth01.txt"), unpack=True)
...
def test_hbf_bhattacharya11():
    d_hbf = np.loadtxt(
        os.path.join(dirdat, "hbf_bhattacharya11.txt"), unpack=True
    )
