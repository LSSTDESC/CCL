...
def test_iswcl():
    ...
    bz = np.ones_like(zs)
    tr_n = ccl.NumberCountsTracer(
        COSMO, has_rsd=False, dndz=(zs, nz), bias=(zs, bz)
    )
    ...
    # H(z)/H0
    ez = ccl.h_over_h0(COSMO, 1.0 / (1 + zs))
    # Linear growth and derivative
    dz = ccl.growth_factor(COSMO, 1.0 / (1 + zs))
