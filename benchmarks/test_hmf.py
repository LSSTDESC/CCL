...
def test_hmf_despali16():
    ...
    d_hmf = np.loadtxt(os.path.join(dirdat, "hmf_despali16.txt"), unpack=True)
    m = d_hmf[0]
    for iz, z in enumerate(zs):
        nm_d = d_hmf[iz + 1]
        nm_h = mf(cosmo, m, 1.0 / (1 + z))
        assert np.all(np.fabs(nm_h / nm_d - 1) < 0.01)


def test_hmf_bocquet16():
    mf = ccl.halos.MassFuncBocquet16(mass_def="200c")
    d_hmf = np.loadtxt(os.path.join(dirdat, "hmf_bocquet16.txt"), unpack=True)
    m = d_hmf[0]
    for iz, z in enumerate(zs):
        nm_d = d_hmf[iz + 1]
        nm_h = mf(cosmo, m, 1.0 / (1 + z))
...
def test_hmf_watson13():
    ...
    d_hmf = np.loadtxt(os.path.join(dirdat, "hmf_watson13.txt"), unpack=True)
    m = d_hmf[0]
    for iz, z in enumerate(zs):
        nm_d = d_hmf[iz + 1]
        nm_h = mf(cosmo, m, 1.0 / (1 + z))
...
def test_hmf_tinker08():
    ...
    d_hmf = np.loadtxt(os.path.join(dirdat, "hmf_tinker08.txt"), unpack=True)
    m = d_hmf[0]
    for iz, z in enumerate(zs):
        nm_d = d_hmf[iz + 1]
        nm_h = mf(cosmo, m, 1.0 / (1 + z))
...
def test_hmf_press74():
    ...
    d_hmf = np.loadtxt(os.path.join(dirdat, "hmf_press74.txt"), unpack=True)
    m = d_hmf[0]
    for iz, z in enumerate(zs):
        nm_d = d_hmf[iz + 1]
        nm_h = mf(cosmo, m, 1.0 / (1 + z))
...
def test_hmf_angulo12():
    ...
    d_hmf = np.loadtxt(os.path.join(dirdat, "hmf_angulo12.txt"), unpack=True)
    m = d_hmf[0]
    for iz, z in enumerate(zs):
        nm_d = d_hmf[iz + 1]
        nm_h = mf(cosmo, m, 1.0 / (1 + z))
...
def test_hmf_sheth99():
    ...
    d_hmf = np.loadtxt(os.path.join(dirdat, "hmf_sheth99.txt"), unpack=True)
    m = d_hmf[0]
    for iz, z in enumerate(zs):
        nm_d = d_hmf[iz + 1]
        nm_h = mf(cosmo, m, 1.0 / (1 + z))
...
def test_hmf_jenkins01():
    ...
    d_hmf = np.loadtxt(os.path.join(dirdat, "hmf_jenkins01.txt"), unpack=True)
    m = d_hmf[0]
    for iz, z in enumerate(zs):
        nm_d = d_hmf[iz + 1]
        nm_h = mf(cosmo, m, 1.0 / (1 + z))
