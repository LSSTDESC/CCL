...
def set_up(request):
    ...
    fl_dl = (ells + 0.5) ** 2 / np.sqrt(
        (ells + 2.0) * (ells + 1.0) * ells * (ells - 1.0)
    )
    fl_ll = fl_dl**2
    fl_lc = (
        ells
...
@pytest.mark.parametrize(
    "t1,t2,bm,a1b1,a1b2,a2b1,a2b2,fl",
    [
        (
            "g1",
            "g1",
...
def test_cells(set_up, t1, t2, bm, a1b1, a1b2, a2b1, a2b2, fl):
    ...
    el = np.sqrt(
        (bmk[a1b1] * bmk[a2b2] + bmk[a1b2] * bmk[a2b1])
        / (2 * lfc["ells"] + 1.0)
    )
...
def test_cells_spline(set_up, t1, t2, bm, a1b1, a1b2, a2b1, a2b2, fl):
    cosmo, trcs, lfc, bmk = set_up
    cl = (
        ccl.angular_cl(
            cosmo,
