...
def test_satellite_shear_profile():
    ...
    sat_gamma_HOD_simps = ccl.halos.SatelliteShearHOD(
        concentration=cM, lmax=6, a1h=0.000989, mass_def="200m"
    )
    gamma_k_CCL = sat_gamma_HOD_simps._usat_fourier(
        cosmo, k_arr, mass_eval, 1 / (1 + z_eval)
    )
    ...
    hmc = ccl.halos.HMCalculator(
        mass_function=nM, halo_bias=bM, mass_def="200m", nM=64
    )

    sat_gamma_HOD = ccl.halos.SatelliteShearHOD(
        concentration=cM, mass_def="200m"
    )
    ...
    z_arr = np.linspace(0.0, 3.0, 256)
    z0 = 0.1
    pz = 1.0 / (2.0 * z0) * (z_arr / z0) ** 2.0 * np.exp(-z_arr / z0)
    ...
    cl_GI_1h = ccl.angular_cl(
        cosmo, wl_tracer, ia_tracer, l_arr, p_of_k_a=pk_GI_1h
    )
    cl_II_1h = ccl.angular_cl(
        cosmo, ia_tracer, ia_tracer, l_arr, p_of_k_a=pk_II_1h
    )
    ...
    cosmo.compute_sigma()
    Ns_M_mean = sat_gamma_HOD._Ns(mass_benchmark, 1.0)
