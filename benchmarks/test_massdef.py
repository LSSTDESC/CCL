...
Ms, Rs_200m, Rs_500c, Ms_500c = np.loadtxt(
    os.path.join(dirdat, "mdef_bm.txt"), unpack=True
)
...
def test_mdef_get_radius():
    Rs_200m_h = hmd_200m.get_radius(cosmo, Ms, 1.0)
    Rs_500c_h = hmd_500c.get_radius(cosmo, Ms, 1.0)
    assert np.all(np.fabs(Rs_200m_h / Rs_200m - 1) < 1e-6)
    assert np.all(np.fabs(Rs_500c_h / Rs_500c - 1) < 1e-6)


def test_mdef_get_mass():
    Ms_h = hmd_200m.get_mass(cosmo, Rs_200m, 1.0)
    assert np.all(np.fabs(Ms_h / Ms - 1) < 1e-6)
    Ms_h = hmd_500c.get_mass(cosmo, Rs_500c, 1.0)
    assert np.all(np.fabs(Ms_h / Ms - 1) < 1e-6)
...
def test_mdef_translate_mass():
    translator = ccl.halos.mass_translator(
        mass_in=hmd_200m, mass_out=hmd_500c, concentration="Duffy08"
    )
    Ms_500c_h = translator(cosmo, Ms, 1.0)
