...
def set_up():
    cosmo=ccl.Cosmology(Omega_c=0.30, Omega_b=0.00, Omega_g=0, Omega_k=0,
                        ...
                        mass_function='tinker', matter_power_spectrum='linear')
    ...
    zmean_2 = 1.5
    sigz_2 = 0.15
    z1, a1=np.loadtxt("./tests/benchmark/codecomp_step2_outputs/cclamparranalytic1nz512nb.txt",
                      unpack=True)
    z2, a2=np.loadtxt("./tests/benchmark/codecomp_step2_outputs/cclamparranalytic2nz512nb.txt",
                      unpack=True)
    ...
    # Histogram case
    arrays['histo'] = {}
    z1, pz1 = np.loadtxt("./tests/benchmark/codecomp_step2_outputs/bin1_histo.txt",
                         unpack=True)[:,1:]
    zz1, a1 = np.loadtxt("./tests/benchmark/codecomp_step2_outputs/cclamparrhisto1nznb.txt",
                         unpack=True)
    z2, pz2 = np.loadtxt("./tests/benchmark/codecomp_step2_outputs/bin2_histo.txt",
                         unpack=True)[:,1:]
    _, a2 = np.loadtxt("./tests/benchmark/codecomp_step2_outputs/cclamparrhisto2nznb.txt",
                       unpack=True)
    arrays['histo']['z1'] = z1
    arrays['histo']['z2'] = z2
    arrays['histo']['a1'] = a1
...
def check_cls(typ):
    ...
    # Generate tracers
    a = arrs[typ]
    g1 = ccl.NumberCountsTracer(cosmo, False, (a['z1'], a['p1']), (a['z1'], a['bz']))
    g2 = ccl.NumberCountsTracer(cosmo, False, (a['z2'], a['p2']), (a['z2'], a['bz']))
    l1 = ccl.WeakLensingTracer(cosmo, (a['z1'], a['p1']))
    l2 = ccl.WeakLensingTracer(cosmo, (a['z2'], a['p2']))
    i1 = ccl.WeakLensingTracer(cosmo, (a['z1'], a['p1']), has_shear=False,
                               ia_bias=(a['z1'], a['a1']), red_frac=(a['z1'], a['rz']))
    i2 = ccl.WeakLensingTracer(cosmo, (a['z2'], a['p2']), has_shear=False,
                               ia_bias=(a['z2'], a['a2']), red_frac=(a['z2'], a['rz']))
    ...
    fl_one = np.ones(nls)
    fl_dl = (ells + 0.5)**2/np.sqrt((ells + 2.) * (ells + 1.) * ells * (ells - 1.))
    fl_ll = fl_dl**2
    fl_lc = ells * (ells + 1)/np.sqrt((ells + 2.) * (ells + 1.) * ells * (ells - 1.))
    fl_li = 2 * fl_dl
    ...
    cl_li_12 = read_bm(pre + 'b1b2' + post + 'li.txt')
