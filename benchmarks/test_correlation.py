...
def set_up(request):
    ...
    trc["g1"] = ccl.NumberCountsTracer(
        cosmo, has_rsd=False, dndz=(z1, pz1), bias=(z2, bz)
    )
    trc["g2"] = ccl.NumberCountsTracer(
        cosmo, has_rsd=False, dndz=(z2, pz2), bias=(z2, bz)
    )
    trc["l1"] = ccl.WeakLensingTracer(cosmo, dndz=(z1, pz1))
    trc["l2"] = ccl.WeakLensingTracer(cosmo, dndz=(z2, pz2))
    trc["i1"] = ccl.WeakLensingTracer(
        cosmo, dndz=(z1, pz1), has_shear=False, ia_bias=(z1, a1)
    )
    trc["i2"] = ccl.WeakLensingTracer(
        cosmo, dndz=(z2, pz2), has_shear=False, ia_bias=(z2, a2)
    )
    trc["ct"] = ccl.CMBLensingTracer(cosmo, z_source=1100.0)
    ...
    ers["dd_11"] = interp1d(
        d[0] / 60.0, d[1], fill_value=d[1][0], bounds_error=False
    )(theta)
    ers["dd_22"] = interp1d(
        d[0] / 60.0, d[2], fill_value=d[2][0], bounds_error=False
    )(theta)
    ...
    ers["dl_22"] = interp1d(
        d[0] / 60.0, d[3], fill_value=d[3][0], bounds_error=False
    )(theta)
    ers["dl_21"] = interp1d(
        d[0] / 60.0, d[4], fill_value=d[4][0], bounds_error=False
    )(theta)
...
def test_xi(set_up, corr_method, t1, t2, bm, er, kind, pref):
    ...
    t0 = time.time()
    xi = ccl.correlation(
        cosmo, ell=ell, C_ell=cli, theta=bms["theta"], type=kind, method=method
    )
    T0 += time.time() - t0
