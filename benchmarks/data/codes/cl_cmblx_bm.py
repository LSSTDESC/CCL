...
cosmo = cosmo.clone(
    ...
    Ob0=cosmo_fid["Omb"],
    Om0=cosmo_fid["Om"],
    m_nu=cosmo_fid["m_nu"] * u.eV,
    Neff=cosmo_fid["Neff"],
    Tcmb0=cosmo_fid["Tcmb0"],
)
cosmo_h = cosmo.clone(H0=100)

pk_params = {"non_linear": 0, "kmax": 1e3, "kmin": 5.0e-5, "nk": 5000}
...
class Power_Spectra:
    ...
    def DZ_int(
        ...
        for i in z:
            Dz[j] = (
                cosmo.H(i).value
                * scipy_int1d(intf, i, np.inf, epsrel=1.0e-6, epsabs=1e-6)[0]
            )
            j = j + 1
    ...
    def sigma_crit(self, zl=[], zs=[], cosmo_h=None):
        ...
        ddls = 1 - np.multiply.outer(1.0 / ds, dl)  # (ds-dl)/ds
        w = (
            (3.0 / 2.0)
            * ((cosmo_h.H0 / c) ** 2)
            * (1 + zl)
            * dl
            / self.Rho_crit(cosmo_h)
        )
        sigma_c = 1.0 / (ddls * w)
        x = ddls <= 0  # zs<zl
        sigma_c[x] = np.inf
    ...
    def ccl_pk_bbks(self, z, cosmo_params=None, pk_params=None):
        ...
        cosmo_ccl = pyccl.Cosmology(
            h=cosmo_params["h"],
            Omega_c=cosmo_params["Omc"],
            Omega_b=cosmo_params["Omb"],
            sigma8=cosmo_params["s8"],
            n_s=cosmo_params["ns"],
        ...
        kh = np.logspace(
            np.log10(pk_params["kmin"]),
            np.log10(pk_params["kmax"]),
            pk_params["nk"],
        )
        nz = len(z)
        ps = np.zeros((nz, pk_params["nk"]))
        ps0 = []
        z0 = 9.0  # PS(z0) will be rescaled using growth function when CCL fails.
        ...
        for i in np.arange(nz):
            try:
                ps[i] = pyccl_pkf(cosmo_ccl, kh, 1.0 / (1 + z[i]))
            except Exception as err:
                print("CCL err", err, z[i])
                if not np.any(ps0):
                    ps0 = pyccl.linear_matter_power(
                        cosmo_ccl, kh, 1.0 / (1.0 + z0)
                    )
        ...
        return (
            ps * cosmo_params["h"] ** 3,
            kh / cosmo_params["h"],
        )  # factors of h to get in same units as camb output
    ...
    def cl_z(
        ...
        for i in np.arange(nz):
            DC_i = cosmo_h.comoving_transverse_distance(
                z[i]
            ).value  # because camb k in h/mpc
            lz = kh * DC_i - 0.5
            DC_i = cosmo_h.comoving_transverse_distance(z[i]).value
            pk_int = interp1d(
                lz, pk[i] / DC_i**2, bounds_error=False, fill_value=0
            )
    ...
    def kappa_cl(
        ...
        if log_zl:  # bins for z_lens.
            zl = np.logspace(
                np.log10(max(zl_min, 1.0e-4)), np.log10(zl_max), n_zl
            )
        else:
            zl = np.linspace(zl_min, zl_max, n_zl)

        clz = self.cl_z(
            z=zl, l=l, cosmo_h=cosmo_h, pk_params=pk_params, pk_func=pk_func
        )
        ...
        sigma_c1 = rho / self.sigma_crit(
            zl=zl, zs=zs1, cosmo_h=cosmo_h
        )  # 3/2 H0^2 (1+zl) chi(zl) * (1 - chi(zl)/chi(zs)) [n_zs,n_zl]
        ...
        f = l * (l + 1.0) / (l + 0.5) ** 2  # correction from Kilbinger+ 2017
        cl *= f**2
    ...
    def g_kappa_cl(
        ...
        clz = self.cl_z(
            z=zg, l=l, cosmo_h=cosmo_h, pk_params=pk_params, pk_func=pk_func
        )  # p(k)/chi**2
        clz *= bias_g
...
if __name__ == "__main__":
    ...
    # Binned 1
    zg = np.genfromtxt(
        "../codecomp_step2_outputs/bin1_histo.txt", names=("z", "pz")
    )
    l, cl_g_cmb = PS.g_kappa_cl(
        pk_func=PS.ccl_pk_bbks,
        zs=[1100],
        p_zs=[1],
        zg=zg["z"],
        p_zg=zg["pz"],
    ...
    np.savetxt(
        "../codecomp_step2_outputs/run_b1b1histo_log_cl_dc.txt",
        np.column_stack((l, cl_g_cmb)),
        fmt=["%i", "%.18e"],
    )

    # Binned 2
    zg = np.genfromtxt(
        "../codecomp_step2_outputs/bin2_histo.txt", names=("z", "pz")
    )
    l, cl_g_cmb = PS.g_kappa_cl(
        pk_func=PS.ccl_pk_bbks,
        zs=[1100],
    ...
    np.savetxt(
        "../codecomp_step2_outputs/run_b2b2histo_log_cl_dc.txt",
        np.column_stack((l, cl_g_cmb)),
        fmt=["%i", "%.18e"],
    )
    ...
    # Analytic
    sigz1 = 0.15
    zav = np.linspace(0, 10.0, 10000)
    pza = (
        1.0
        / (np.sqrt(2 * math.pi) * sigz1)
        * np.exp(-0.5 * ((zav - 1) / sigz1) ** 2)
    )
    za = {"z": zav, "pz": pza}
    l, cl_g_cmb = PS.g_kappa_cl(
        pk_func=PS.ccl_pk_bbks,
    ...
    np.savetxt(
        "../codecomp_step2_outputs/run_b1b1analytic_log_cl_dc.txt",
        np.column_stack((l, cl_g_cmb)),
        fmt=["%i", "%.18e"],
    )
    ...
    np.savetxt(
        "../codecomp_step2_outputs/run_b2b2analytic_log_cl_dc.txt",
        np.column_stack((l, cl_g_cmb)),
        fmt=["%i", "%.18e"],
    )
    ...
    np.savetxt(
        "../codecomp_step2_outputs/run_b1b1histo_log_cl_lc.txt",
        np.column_stack((l, cl_g_cmb)),
        fmt=["%i", "%.18e"],
    )
    ...
    np.savetxt(
        "../codecomp_step2_outputs/run_b2b2histo_log_cl_lc.txt",
        np.column_stack((l, cl_g_cmb)),
        fmt=["%i", "%.18e"],
    )
    ...
    np.savetxt(
        "../codecomp_step2_outputs/run_b1b1analytic_log_cl_lc.txt",
        np.column_stack((l, cl_g_cmb)),
        fmt=["%i", "%.18e"],
    )
    ...
    np.savetxt(
        "../codecomp_step2_outputs/run_b2b2analytic_log_cl_lc.txt",
        np.column_stack((l, cl_g_cmb)),
        fmt=["%i", "%.18e"],
    )
