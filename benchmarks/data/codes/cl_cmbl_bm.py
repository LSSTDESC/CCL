...
cosmo = csmm.FlatLambdaCDM(70.0, 0.3, Tcmb0=2.725)
...
pk_params = {"non_linear": 1, "kmax": 30, "kmin": 1.0e-4, "nk": 5000}
...
class Power_Spectra:
    ...
    def cl_z(
        ...
        for i in np.arange(nz):
            ...
            cls[i][:] += (
                pk_int(l) * u.Mpc * (c / (cosmo_h.efunc(z[i]) * cosmo_h.H0))
            )
        return cls
    ...
    def kappa_cl(
        ...
        sigma_c1 = (
            prefac * (1 + zl) * dl * (1 - np.multiply.outer(1.0 / ds1, dl))
        )
        sigma_c2 = (
            prefac * (1 + zl) * dl * (1 - np.multiply.outer(1.0 / ds2, dl))
        )
        clz = self.cl_z(
...
if __name__ == "__main__":
    PS = Power_Spectra()
    l, cl2 = PS.kappa_cl(
        n_zl=10000, log_zl=True, zl_min=1.0e-4, zl_max=1100, pk_func=PS.ccl_pk
    )
