...
def st_gs(sigma, dc):
    ...
    return (
        params["ST_A"]
        * np.sqrt(2 * params["ST_a"] / np.pi)
        * (1 + (1.0 / anu2) ** params["ST_p"])
        * nu
        * expnu
    )
...
def st_b1(sigma, dc):
    ...
    return (
        1
        + (anu2 - 1 + 2 * params["ST_p"] / (1 + anu2 ** params["ST_p"]))
        / params["DELTA_C"]
    )
...
def get_halomod_pk(cp, karr, z, integrate=True, fname_out=None):
    ...
    lmarr = (
        LMMIN + (LMMAX - LMMIN) * (np.arange(NM) + 0.5) / NM
    )  # log(M*h/M_sum)
    dlm = (LMMAX - LMMIN) / NM
    lmarrb = np.zeros(NM + 2)
    lmarrb[0] = lmarr[0] - dlm
    lmarrb[1:-1] = lmarr
    lmarrb[-1] = lmarr[-1] + dlm
    marr = 10.0**lmarr  # M (in Msun/h) M_h Ms/h = M Ms
    marrb = 10.0**lmarrb  # M (in Msun/h) M_h Ms/h = M Ms
    sigmarrb = ccl.sigmaM(cosmo, marrb / cp["h"], 1.0 / (1 + z))
    sigmarr = sigmarrb[1:-1]
    dlsMdl10M = np.fabs((sigmarrb[2:] - sigmarrb[:-2]) / (2 * dlm)) / sigmarr
    omega_z = (
        cp["Om_m"]
        * (1 + z) ** 3
        / (cp["Om_m"] * (1 + z) ** 3 + 1 - cp["Om_m"])
    )
    delta_c = params["DELTA_C"] * (1 + 0.012299 * np.log10(omega_z))
    Delta_v = (
        18 * np.pi**2 + 82 * (omega_z - 1) - 39 * (omega_z - 1) ** 2
    ) / omega_z
    fM = st_gs(sigmarr, delta_c) * dlsMdl10M
    bM = st_b1(sigmarr, delta_c)
    cM = duffy_c(marr, z)
    cMf = interp1d(
        lmarr, cM, kind="linear", bounds_error=False, fill_value=cM[0]
    )
    rhoM = ccl.rho_x(cosmo, 1.0, "matter") / cp["h"] ** 2
    ...
    rsM = rvM / cM
    rsM0 = (
        3 * 10.0**LM0 / (4 * np.pi * rhoM * Delta_v)
    ) ** 0.333333333 / duffy_c(10.0**LM0, z)
    rsMf = interp1d(
        lmarr, rsM, kind="linear", bounds_error=False, fill_value=rsM0
    )
    ...
    f1h0 = fM0 * 10.0**LM0 / rhoM
    f2hf = interp1d(
        lmarr, fM * bM, kind="linear", bounds_error=False, fill_value=0
    )
    f2h0 = fbM0
    ...
    pklin = (
        ccl.linear_matter_power(cosmo, karr * cp["h"], 1.0 / (1 + z))
        * cp["h"] ** 3
    )
...
k_arr = 10.0 ** (LKHMIN + (LKHMAX - LKHMIN) * (np.arange(NKH) + 0.5) / NKH)
for ip, cp in enumerate([cpar1, cpar2, cpar3]):
    for z in [0, 1]:
        get_halomod_pk(
            ...
            z + 0.0,
            integrate=USE_INT,
            fname_out="pk_hm_c%d_z%d.txt" % (ip + 1, z),
        )
