...
ZVALS = np.arange(0.0, 3.0, 0.5)  # Redshifts to evaluate P(k) at
...
if not os.path.exists(class_datadir):
    os.makedirs(class_datadir)
if not os.path.exists(ccl_datadir):
    os.makedirs(ccl_datadir)
...
sample_points = generate_latin_hypercube(
    samples=NSAMP, param_dict=param_dict, class_root=CLASS_ROOT, seed=SEED
)
...
generate_class_ini(
    sample_points, root="%s_lin_std" % root, nonlinear=False, redshifts=ZVALS
)
generate_class_ini(
    sample_points, root="%s_lin_pre" % root, nonlinear=False, redshifts=ZVALS
)

print("Writing CLASS nonlinear .ini files")
generate_class_ini(
    sample_points, root="%s_nl_std" % root, nonlinear=True, redshifts=ZVALS
)
generate_class_ini(
    sample_points, root="%s_nl_pre" % root, nonlinear=True, redshifts=ZVALS
)
...
run_class(
    fname_pattern="%s_lin_std_?????.ini" % root,
    class_root=CLASS_ROOT,
    precision=False,
)
run_class(
    fname_pattern="%s_lin_pre_?????.ini" % root,
    class_root=CLASS_ROOT,
    precision=True,
)
run_class(
    fname_pattern="%s_nl_std_?????.ini" % root,
    class_root=CLASS_ROOT,
    precision=False,
)
run_class(
    fname_pattern="%s_nl_pre_?????.ini" % root,
    class_root=CLASS_ROOT,
    precision=True,
)
