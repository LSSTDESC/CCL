...
lkmax = 2
n_per_decade = 20
z_s = np.array([0.0, 1.0])
a_s = 1.0 / (1 + z_s)
ks = np.logspace(lkmin, lkmax, (lkmax - lkmin) * n_per_decade)
pk = ccl.linear_matter_power(cosmo, ks, 1.0)
...
dd_bias = pt_ob.one_loop_dd_bias_b3nl(pk, P_window=P_window, C_window=C_window)
...
pgi = b1 * (c1 * Pd1d1 + cd * (a00e + c00e) + c2 * (a0e2 + b0e2))
pii = (
    c1**2 * Pd1d1
    + 2 * c1 * cd * (a00e + c00e)
    + cd**2 * a0e0e
...
pii_bb = cd**2 * a0b0b + c2**2 * ab2b2 + 2 * cd * c2 * d0bb2
pim = c1 * Pd1d1 + cd * (a00e + c00e) + c2 * (a0e2 + b0e2)

np.savetxt(
    "../pt_bm_z0.txt",
    np.transpose([ks, pgg[0], pgm[0], pgi[0], pii[0], pii_bb[0], pim[0]]),
    header="[0]-k  [1]-GG [2]-GM [3]-GI [4]-II [5]-II_BB [6]-IM",
)
np.savetxt(
    "../pt_bm_z1.txt",
    np.transpose([ks, pgg[1], pgm[1], pgi[1], pii[1], pii_bb[1], pim[1]]),
    header="[0]-k  [1]-GG [2]-GM [3]-GI [4]-II [5]-II_BB [6]-IM",
)
