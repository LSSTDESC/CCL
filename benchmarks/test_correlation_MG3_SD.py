...
def set_up(request):
    ...
    # Initialize tracers
    trc = {}
    trc["g1"] = ccl.NumberCountsTracer(
        cosmo, has_rsd=False, dndz=(z1, pz1), bias=(z1, bz1)
    )
    trc["g2"] = ccl.NumberCountsTracer(
        cosmo, has_rsd=False, dndz=(z2, pz2), bias=(z2, bz2)
    )
    ...
    bms["dd_22"] = np.loadtxt(dirdat + "/wtheta_linear_predictionSD.dat")[
        15:30
    ]
    bms["dl_11"] = np.loadtxt(dirdat + "/gammat_linear_predictionSD.dat")[0:15]
    bms["dl_12"] = np.loadtxt(dirdat + "/gammat_linear_predictionSD.dat")[
        15:30
    ]
    bms["dl_21"] = np.loadtxt(dirdat + "/gammat_linear_predictionSD.dat")[
        30:45
    ]
    bms["dl_22"] = np.loadtxt(dirdat + "/gammat_linear_predictionSD.dat")[
        45:60
    ]
    ...
    ers["dd_11"] = interp1d(
        d[0], d[1], fill_value=d[1][0], bounds_error=False
    )(theta)
    ers["dd_22"] = interp1d(
        d[0], d[2], fill_value=d[2][0], bounds_error=False
    )(theta)
    ...
    ers["dl_22"] = interp1d(
        d[0], d[3], fill_value=d[3][0], bounds_error=False
    )(theta)
    ers["dl_21"] = interp1d(
        d[0], d[4], fill_value=d[4][0], bounds_error=False
    )(theta)
    d = np.loadtxt("benchmarks/data/sigma_xi+_Nbin5", unpack=True)
    # We cut the largest theta angle from xip because of issues
    # with the benchmark.
    ers["ll_11_p"] = interp1d(
        d[0], d[1], fill_value=d[1][0], bounds_error=False
    )(theta)
    ers["ll_22_p"] = interp1d(
        d[0], d[2], fill_value=d[2][0], bounds_error=False
    )(theta)
    ers["ll_12_p"] = interp1d(
        d[0], d[3], fill_value=d[3][0], bounds_error=False
    )(theta)
    d = np.loadtxt("benchmarks/data/sigma_xi-_Nbin5", unpack=True)
    ers["ll_11_m"] = interp1d(
        d[0], d[1], fill_value=d[1][0], bounds_error=False
    )(theta)
    ers["ll_22_m"] = interp1d(
        d[0], d[2], fill_value=d[2][0], bounds_error=False
    )(theta)
    ers["ll_12_m"] = interp1d(
        d[0], d[3], fill_value=d[3][0], bounds_error=False
    )(theta)
...
def test_xi(set_up, corr_method, t1, t2, bm, er, kind, pref):
    ...
    theta_deg = bms["theta"] / 60.0
    xi = ccl.correlation(
        cosmo, ell=ell, C_ell=cli, theta=theta_deg, type=kind, method=method
    )
    xi *= pref
