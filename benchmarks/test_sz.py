...
def test_szcl():
    ...
    bm = np.loadtxt(
        "benchmarks/data/sz_cl_P13_szpowerspectrum.txt", unpack=True
    )
    l_bm = bm[0]
    cl_bm = bm[1]
    ...
    prf.update_parameters(mass_bias=1.0 / 1.41, x_out=6.0)
    tr = ccl.tSZTracer(COSMO, z_max=3.0)
    ...
    a_arr = 1.0 / (1 + np.linspace(0, 3.0, 20))[::-1]
    tkk = ccl.halos.halomod_Tk3D_1h(
        COSMO, hmc, prf, lk_arr=lk_arr, a_arr=a_arr, use_log=True
    )
    tll = ccl.angular_cl_cov_cNG(
        COSMO, tr, tr, ell=l_bm, t_of_kk_a=tkk, fsky=fsky
    )
