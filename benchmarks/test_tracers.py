...
def get_prediction(ells,chi_i,chi_f,alpha,beta,gamma,
    ...
    if der_angles==2:
        ...
        fl*=((ells+1.)*ells)**2
    return fl*(ells+0.5)**(alpha+2*beta)*(chi_f**exponent-chi_i**exponent)/exponent
...
@pytest.mark.parametrize("alpha,beta,gamma,is_factorizable,w_transfer,mismatch,der_bessel,der_angles",
                         [
                          (-2.,-1.,-1.,False,True,False,0,0),  # non-factorizable transfer function
                          (-2., 0.,-1.,False,True,False,0,0),
                          (-2.,-1., 0.,False,True,False,0,0),
                          (-2., 0., 0.,False,True,False,0,0),
                          ( 0., 0., 0.,False,True,False,0,0),
                          (-2.,-1.,-1.,True,True,False,0,0),  # factorizable transfer functions
                          (-2., 0.,-1.,True,True,False,0,0),
                          (-2.,-1., 0.,True,True,False,0,0),
                          (-2., 0., 0.,True,True,False,0,0),
                          ( 0., 0., 0.,True,True,False,0,0),
                          (-2., 0., 0.,False,False,False,0,0),  # Unit transfer functions
                          ( 0., 0., 0.,False,False,False,0,0),
                          (-2.,-1., 0.,True,True,True,0,0),  # Mismatch in kernel-transfer coverage
                          (-2.,-1., 0.,False,True,True,0,0),
                          (-2.,-1.,-1.,False,True,False,-1,0),  # non-zero der_bessel
                          (-2.,-1.,-1.,False,True,False,0,1),  # non-zero der_angles
...
def test_tracers_analytic(set_up,alpha,beta,gamma,
    ...
    if mismatch:
        mask=(chiarr<0.9*chif) & (chiarr>1.1*chii)  # Remove elements around the edges
    ...
    if w_transfer:
        ...
        if is_factorizable:
            ...
            t.add_tracer(cosmo,kernel=(chiarr,wchi),transfer_k=(lkarr,tk),
                         transfer_a=(aarr_transfer,ta),der_bessel=der_bessel,der_angles=der_angles)
        else:
            # 2D
            tka=ta[:,None]*tk[None,:]
            t.add_tracer(cosmo,kernel=(chiarr,wchi),
                         transfer_ka=(aarr_transfer,lkarr,tka),
                         der_bessel=der_bessel,der_angles=der_angles)
    else:
        t.add_tracer(cosmo,kernel=(chiarr,wchi),der_bessel=der_bessel,der_angles=der_angles)
    ...
    # Prediction
    clpred = get_prediction(larr,chii,chif,alpha,beta,gamma,der_bessel,der_angles)
