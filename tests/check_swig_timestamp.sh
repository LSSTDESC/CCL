...
t_swig_pylib=`git log --pretty=format:%ct -n 1 ../pyccl/ccllib.py`
t_swig_cwrap=`git log --pretty=format:%ct -n 1 ../pyccl/ccl_wrap.c`
...
do
    ...
    if [ "$last_commit" -gt "$t_swig_pylib" ] || [ "$last_commit" -gt "$t_swig_cwrap" ];
then
    echo $msg1
    echo ""
    echo $msg2
...
echo "Autogenerated SWIG files appear to be up to date."
