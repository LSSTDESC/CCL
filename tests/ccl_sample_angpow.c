...
int main(int argc,char **argv)
{
  ...
  CCL_ClTracer *ct_gc_A=ccl_cl_tracer_number_counts_new(cosmo,1,0,NZ,z_arr_gc,nz_arr_gc,
							NZ,z_arr_gc,bz_arr,-1,NULL,NULL,&status);
  CCL_ClTracer *ct_gc_B=ccl_cl_tracer_number_counts_new(cosmo,1,0,NZ,z_arr_gc,nz_arr_gc,
							NZ,z_arr_gc,bz_arr,-1,NULL,NULL,&status);
  ...
  double dlk = 0.003;
  double zmin = 0.05;
  CCL_ClWorkspace *wyl=ccl_cl_workspace_new(NL+1,-1          ,CCL_NONLIMBER_METHOD_ANGPOW,logstep,linstep,dchi,dlk,zmin,&status);
  ...
  printf("chimin: %.3f chimax: %.3f dchi: %.3f nsteps: %d\n",ct_gc_A->chimin,ct_gc_A->chimax,dchi,(int)((ct_gc_A->chimax-ct_gc_A->chimin)/dchi)+1);
  //printf("Bchimin: %.3f chimax: %.3f dchi: %.3f nsteps: %d\n",ct_gc_B->chimin,ct_gc_B->chimax,dchi,(int)((ct_gc_B->chimax-ct_gc_B->chimin)/dchi)+1);
  start = clock();
